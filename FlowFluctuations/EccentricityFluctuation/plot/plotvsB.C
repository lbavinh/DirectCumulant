...
void plotvsB(TString model ="UrQMD", TString energy = "4.5GeV") {
  
  TString inFileName = (TString) Form("../ROOTFile/%s_%s.root",model.Data(),energy.Data());
  // TString inFileName = "../ROOTFile/output_old.root";
  TFile *outFile = new TFile(Form("./ecc_%s_%s_vsB.root",model.Data(),energy.Data()),"recreate");
  ...
  const double maxcent = 16.4; // for v2 vs centrality
  ...
  // Histogram input
  TProfile *prEcc22 = (TProfile*)inFile->Get(Form("prEcc22vsB6"));
  TProfile *prEcc24 = (TProfile*)inFile->Get(Form("prEcc24vsB6"));
  TProfile *prCov24 = (TProfile*)inFile->Get(Form("prCov24vsB6"));
  TProfile *ecc2vsCent = (TProfile *) inFile -> Get("prEcc2vsB6");
  ...
  leg->Draw();
  c->cd(2);
  TH2F *hRatio = new TH2F("hRatio",Form(";b, fm;Ratio to #epsilon_{2}{2}"),ncent,mincent,maxcent,1,0.3,1.4);
  ...
  c->SaveAs(Form("Eccentricity2_%s_%s_vsB.png",model.Data(),energy.Data()));
  delete prEcc22;
  delete prEcc24;
  delete prCov24;
  inFile->Close();
