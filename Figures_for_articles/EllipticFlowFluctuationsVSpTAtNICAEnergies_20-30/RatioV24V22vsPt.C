void RatioV24V22vsPt(){
  ...
  const float leg_coordinate[4]={0.3,0.05,0.6,0.2}; //  0.05,0.7,0.35,0.99
  ...
  TString legendEntries[2]={"STAR", "Model"};
    ...
    for (int imod=0;imod<nmodel;imod++){
      for (int icent=0;icent<ncent;icent++){
        ...
        if      (ien==2 && imod!=4)            grRatioV2[ien][imod][icent] = new TGraphErrors(2,xx,yy,xx,yy); // at 19.6 GeV only have vHLLE+UrQMD
          ...
          for (int i=0;i<nbins[ien][imod][ratioToMethod][icent];i++){
            Double_t ratio = vy_gr[ien][imod][1][icent][i]/vy_gr[ien][imod][ratioToMethod][icent][i];
            Double_t ratioErr = ratio*(TMath::Sqrt(TMath::Power(ey_gr[ien][imod][ratioToMethod][icent][i]/vy_gr[ien][imod][ratioToMethod][icent][i],2)+TMath::Power(ey_gr[ien][imod][1][icent][i]/vy_gr[ien][imod][1][icent][i],2)));
            vRatioV22GappedV24.push_back(ratio);
            vRatioV22GappedV24Err.push_back(ratioErr);
          }
          grRatioV2[ien][imod][icent] = new TGraphErrors(nbins[ien][imod][ratioToMethod][icent],vx_gr[ien][imod][ratioToMethod][icent],&vRatioV22GappedV24[0],ex_gr[ien][imod][ratioToMethod][icent],&vRatioV22GappedV24Err[0]);      
        }
      }
    }
  }
  ...
  for (int imod=0;imod<nmodel;imod++){
    ...
    for (int ien=0;ien<nenergy;ien++){
      ...
      grRatioV2[ien][imod][0] -> SetMarkerColor(kGreen+3);
      grRatioV2[ien][imod][0] -> SetLineColor(kGreen+3);
      ...
      grRatioV2[ien][imod][2] -> SetMarkerStyle(kOpenCircle);
      grRatioV2[ien][imod][2] -> SetMarkerColor(kBlack);
      grRatioV2[ien][imod][2] -> SetLineColor(kBlack);
  ...
  for (int imod=0;imod<nmodel;imod++){
    ...
    if (ien==0 && imod==0) h[imod][ien] = new TH2F(Form("pad_%i_%i",imod,ien),";;v_{2}{4}/v_{2}{2,|#Delta#eta|>0.1}",1,minpt,maxpt,1,minV2Ratio,maxV2Ratio);
    ...
    if (imod==1 && ien==0) {
      ...
      leg_pt->AddEntry(grRatioV2[ien][imod][2],legendEntries[1].Data(),"p");
  ...
  can->SaveAs("Figure_Ratiov24v22_vs_pt_20-30.pdf");
  gROOT->SetStyle("Pub");
  can->SaveAs("Figure_Ratiov24v22_vs_pt_20-30.png");
