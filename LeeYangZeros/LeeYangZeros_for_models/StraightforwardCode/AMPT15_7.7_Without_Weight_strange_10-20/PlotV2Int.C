...
void PlotV2Int(TString inputFileName1 = "FirstRun.root") // TString inputFileName2 = "SecondRun.root"
{
  // bool bUseProduct = 1;
  bool bDebug = 1;
  ...
  for (int i=1; i<6; i++)
  {
    if (!bUseProduct && i==5) continue;
    if (i!=ratioToMethod && i!=1) hRatioInt[i]->Draw("P same");
  }
  TLine lineOne1;
  ...
  for (int i=1; i<5; i++){if (i!=ratioToMethod && i!=1) leg3->AddEntry(hRatioInt[i],methodName[i].Data(),"p");}
...
//   c3.Divide(5,2,0,0);
//   // gROOT->SetStyle("Pub");
//   for (int centrality = 3; centrality < 8; centrality++){
//   c3.cd(centrality-2);
