...
void res2() {
  ...
  for (int ic = 0; ic < ncent; ic++) {
    ...
    eres[ic] = h3->GetBinError(1+ic)/(2*res2EW[ic]);
    ecent[ic] = 0.0;
  ...
  char title[800];
  sprintf(title, "EP resolution, 0.1<|#eta|<2.5");
  ...
  gr1->SetTitle("EP resolution, 0.1<|#eta|<2.5;Centrality, %;R_{2}{#Psi_{2}}");
  ...
  legC12->AddEntry(gr1, "EP 0.1<|#eta|<2.5", "lp");
  legC12->SetFillColor(0);
  legC12->SetBorderSize(0);
  legC12->Draw();
  c1 -> SaveAs("../Graphics/res2.png");

  outfile = new TFile("../ROOTFile/TGraph_res2.root","recreate");
  outfile -> cd();
  gr1->Write("res2");
  outfile -> Close();
