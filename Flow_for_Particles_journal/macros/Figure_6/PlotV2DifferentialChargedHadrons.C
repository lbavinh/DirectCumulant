...
void PlotV2DifferentialChargedHadrons(TString inputFirstRunFileName = input1, TString inputSecondRunFileName = input2)
{
  ...
  bool bUseProduct = 1;
  Int_t nmethod = 8;
  TString title[]={"AMPT, #it{#sigma}_{p}=1.5 mb","#it{v}_{2}^{SP}{Q_{2,TPC}}","#it{v}_{2}{2,#eta-gap}","#it{v}_{2}{2}","AMPT, #it{#sigma}_{p}=1.5 mb","#it{v}_{2}{LYZ, Sum}","#it{v}_{2}{LYZ}","STAR"};
  ...
  const double v2EtaSubSTAR115_10_20[16] = {0.,0.0211,0.0356,0.0483,0.0612,0.0736,0.0855,0.0986,0.1061,0.1178,0.1297,0.1351,0.1396,0.1463,0.1523,0.};
  const double v2EtaSubeSTAR115_10_20[16] = {0.,0.0002,0.0002,0.0003,0.0004,0.0005,0.0007,0.001,0.0013,0.0018,0.0025,0.0035,0.0051,0.007,0.008,0.};
  ...
  const double v2EtaSubSTAR115_30_40[16] = {0.,0.0295,0.0514,0.0717,0.0908,0.1097,0.1273,0.145,0.1585,0.174,0.181,0.1907,0.1962,0.2224,0.2169,0.};
  const double v2EtaSubeSTAR115_30_40[16] = {0.,0.0003,0.0004,0.0005,0.0007,0.0009,0.0012,0.0016,0.0022,0.0031,0.0044,0.0061,0.0091,0.0128,0.0148,0.};
  const double zeroArray[16]={0.};

  for (int i=0;i<ncent;i++) if (i!=2||i!=3||i!=4)graph[i][7] = new TGraphErrors(16,pTBinSTAR,zeroArray,zeroArray,zeroArray);
  graph[2][7] = new TGraphErrors(16,pTBinSTAR,v2EtaSubSTAR115_10_20,zeroArray,v2EtaSubeSTAR115_10_20);
  graph[3][7] = new TGraphErrors(16,pTBinSTAR,v2EtaSubSTAR115_20_30,zeroArray,v2EtaSubeSTAR115_20_30);
  graph[4][7] = new TGraphErrors(16,pTBinSTAR,v2EtaSubSTAR115_30_40,zeroArray,v2EtaSubeSTAR115_30_40);
  // QCumulant
  ...
  TPaveLabel* label = new TPaveLabel(0.1,0.95,0.9,0.99,Form("Au+Au at #sqrt{s_{NN}} = %s GeV, Charged Hadrons",energy.Data()));
  ...
  for (int ic=firstCentToDraw; ic<numberOfCentToDraw+firstCentToDraw; ic++)
  {
    ...
    {
    TLegend *leg1 = new TLegend(0.3,0.1,0.8,0.3);
    ...
    for (int i=0; i<vGr[ic].size(); i++)
    {
      vGr[ic].at(i)->SetTitle(";#it{p}_{T}, GeV/c;#it{v}_{2}{#Psi_{2,TPC}}");
    }
  }
  for (int ic=firstCentToDraw; ic<firstCentToDraw+numberOfCentToDraw; ic++)
  {
    ...
    can->cd(padID);
    vGr_ratio[ic].at(7)->Draw("AP PLC PMC");
    if (ic==firstCentToDraw)
    {
    vGr_ratio[ic].at(7)->GetYaxis()->SetTitleSize(titleSize);
    vGr_ratio[ic].at(7)->GetYaxis()->SetLabelSize(labelSize);
    vGr_ratio[ic].at(7)->GetYaxis()->SetTitleFont(textFont);
    vGr_ratio[ic].at(7)->GetYaxis()->SetLabelFont(textFont);
    vGr_ratio[ic].at(7)->GetXaxis()->SetTitleSize(titleSize);
    vGr_ratio[ic].at(7)->GetXaxis()->SetLabelSize(labelSize);
    vGr_ratio[ic].at(7)->GetXaxis()->SetTitleFont(textFont);
    vGr_ratio[ic].at(7)->GetXaxis()->SetLabelFont(textFont);
    vGr_ratio[ic].at(7)->GetYaxis()->SetTitleOffset(titleOffSet);
    vGr_ratio[ic].at(7)->GetXaxis()->SetTitleOffset(1.);
    vGr_ratio[ic].at(7)->GetXaxis()->SetLabelOffset(0.017);
    tex.SetTextSize(titleSize);
    }
    else
    {
    vGr_ratio[ic].at(7)->GetYaxis()->SetTitleSize(titleSize+0.015);
    vGr_ratio[ic].at(7)->GetYaxis()->SetLabelSize(labelSize+0.015);
    vGr_ratio[ic].at(7)->GetYaxis()->SetTitleFont(textFont);
    vGr_ratio[ic].at(7)->GetYaxis()->SetLabelFont(textFont);
    vGr_ratio[ic].at(7)->GetXaxis()->SetTitleSize(titleSize+0.015);
    vGr_ratio[ic].at(7)->GetXaxis()->SetLabelSize(labelSize+0.015);
    vGr_ratio[ic].at(7)->GetXaxis()->SetTitleFont(textFont);
    vGr_ratio[ic].at(7)->GetXaxis()->SetLabelFont(textFont);
    vGr_ratio[ic].at(7)->GetYaxis()->SetTitleOffset(titleOffSet-0.1);
    vGr_ratio[ic].at(7)->GetXaxis()->SetTitleOffset(0.8);
    tex.SetTextSize(titleSize+0.015);
    }
    ...
    vGr_ratio[ic].at(7)->SetTitle(Form(";#it{p}_{T}, GeV/c;data/model", title[ratioToMethod].Data()));
    
    tex.DrawLatex(0.2,maxRatio-0.1,padName[padID-1].Data());
  }
  can->SaveAs(Form("CompareMethods_Diff_v2EtaSub_vs_STAR_at_%s.png",energy.Data()));
  can->SaveAs(Form("CompareMethods_Diff_v2EtaSub_vs_STAR_at_%s.pdf",energy.Data()));
