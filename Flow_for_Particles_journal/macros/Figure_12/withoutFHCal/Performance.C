...
void Performance(){ // 0: v22; 1:v24; 2: v2EP, 3: gapped v22
  ...
  TFile *inputPeter = new TFile("./Data/graphs_reco_vs_geant.root","read");
  // for (int i=0;i<2;i++){
  // grV2[i][0][4][0] = (TGraphErrors*)inputPeter->Get("gr_v2_FHCalEp_mc");
  // grV2[i][1][4][0] = (TGraphErrors*)inputPeter->Get("gr_v2_FHCalEp_reco");
  // }
  // TFile *inputPeter115GeV = new TFile("./Data/graphs_fhcal_11.5gev_hadrons.root","read");
  // for (int i=0;i<2;i++){
  ...
  for (int i=0;i<nenergy;i++){
    input[0][i] = new TFile(Form("./Data/v2_%s_%s.root",model.Data(),energy[i].Data()),"read");
    input[1][i] = new TFile(Form("./Data/v2_%s_%s_Reco_MotherID.root",model.Data(),energy[i].Data()),"read");
  }
  ...
  // for (int imeth=0;imeth<nmethod;imeth++){
    for (int i=0;i<2;i++){
      for (int ien=0;ien<nenergy;ien++){
        ...
        // }
        for (int ilev=0;ilev<nlevel;ilev++){
          Red(grV2[i][ilev][0][ien]);
          Green(grV2[i][ilev][1][ien]);
          Blue(grV2[i][ilev][2][ien]);
          Black(grV2[i][ilev][3][ien]);
          for (int imeth=0;imeth<nmethod;imeth++){
  ...
  TPaveLabel* title = new TPaveLabel(0.1,0.95,0.9,0.98,"Au+Au, 10-40%, Charged Hadrons, UrQMD, reconstructed (GEANT4)");
  ...
  for (int ipad=0;ipad<8;ipad++){
    ...
    for (int ilev=0;ilev<nlevel;ilev++){
        ...
        if (ipad==4) grV2[0][ilev][0][1]->Draw("P"); // PLC PMC // PLC (Palette Line Color) and PMC (Palette Marker Color)
        if (ipad==5) grV2[0][ilev][2][1]->Draw("P"); // PZ
        if (ipad==6) grV2[0][ilev][1][1]->Draw("P");
        if (ipad==7) grV2[0][ilev][3][1]->Draw("P");
        // if (ipad==9) grV2[0][ilev][4][1]->Draw("P");
    }
    // cout << "error here" << endl;
  }   

  can->SaveAs(Form("Performance.pdf"));
  gROOT->SetStyle("Pub");
  can->SaveAs(Form("Performance.png"));
