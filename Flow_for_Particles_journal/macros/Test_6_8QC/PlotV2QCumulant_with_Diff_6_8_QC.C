...
// Flags
bool saveAsPNG = true;
int ratioToMethod = 1; // 2QC, 4QC, 2QC-gapped, 4QC-gapped, 6QC-gapped, 8QC-gapped
...
TString inputFileName = Form("Testing_6_8QC_UrQMD_7.root");

const int nmethod = 7; // 2QC, 4QC, 2QC-gapped, 4QC-gapped, 6QC-gapped, 8QC-gapped, 6QC
...
const int binMaxPtRFP = 9; // 3.0 GeV
const double minptRFP = 0.2;
const double maxptRFP = 3.0;

const double maxpt = 2.1; // for v2 vs pt plotting
const double minpt = -0.1;  // for v2 vs pt plotting
...
const double maxV2dif = 0.15; // for v2 vs pt plotting


vector <Double_t> coordinateLeg = {0.18,0.5,0.45,0.889};
vector<pair<Double_t,Double_t>> rangeRatio = {{0.68,1.32},{0.68,1.32}};
...
int marker[]={23,25,26,20,22,21,20}; // v22,v24,v22eta-sub,v24eta-gap
...
void CalStatErrCent1040(TString model, TString energy,double v2eDif1040[nmethod][npid][npt]){
  ...
  for (int icent=0; icent<ncent; icent++){ // loop over centrality classes
    ...
    hv26Gap[icent] = (TProfile*)inFile->Get(Form("hv26Gap_%i", icent));
    hv28Gap[icent] = (TProfile*)inFile->Get(Form("hv28Gap_%i", icent));
    for(int ipt=0; ipt<npt; ipt++){ // loop over pt bin
      for (int id=0;id<npid-4;id++){
        ...
        hv26ptGap[icent][ipt][id]=(TProfile*)inFile->Get(Form("hv26ptGap_%i_%i_%i",icent,ipt,id));
        hv28ptGap[icent][ipt][id]=(TProfile*)inFile->Get(Form("hv28ptGap_%i_%i_%i",icent,ipt,id));
      }
  ...
  for (int icent=0;icent<ncent;icent++){
    for (int ipt=0;ipt<npt;ipt++){
      for (int id=8;id<npid;id++){
        ...
        hv26ptGap[icent][ipt][id] = (TProfile*)  hv26ptGap[icent][ipt][id-8] -> Clone();
        hv28ptGap[icent][ipt][id] = (TProfile*)  hv28ptGap[icent][ipt][id-8] -> Clone();
        ...
        hv26ptGap[icent][ipt][id] -> Add(hv26ptGap[icent][ipt][id-4]);
        hv28ptGap[icent][ipt][id] -> Add(hv28ptGap[icent][ipt][id-4]);
      }
    }
  }
  ...
  for (int icent=3; icent<5; icent++){ // add 20-30% & 30-40% to 10-20%
    ...
    hv26Gap[2]-> Add(hv26Gap[icent]);
    hv28Gap[2]-> Add(hv28Gap[icent]);
    for(int ipt=0; ipt<npt; ipt++){ // loop over pt bin
      for (int id=0;id<npid;id++){ // loop over pid
        ...
        hv26ptGap[2][ipt][id]->Add(hv26ptGap[icent][ipt][id]);
        hv28ptGap[2][ipt][id]->Add(hv28ptGap[icent][ipt][id]);
      }
    } // end of loop over pt bin
  }
...
void PlotV2QCumulant_with_Diff_6_8_QC(){
  ...
  double v2eDif1040[nmethod][npid][npt]={{{0.}}};
  ...
  for (int icent=0; icent<ncent; icent++){ // loop over centrality classes
    ...
    hv26Gap[icent] = (TProfile*)inFile->Get(Form("hv26Gap_%i", icent));
    hv28Gap[icent] = (TProfile*)inFile->Get(Form("hv28Gap_%i", icent));
    hv26[icent] = (TProfile*)inFile->Get(Form("hv26_%i",icent));
    ...
    for(int ipt=0; ipt<npt; ipt++){ // loop over pt bin
      for (int id=0;id<npid-4;id++){
        ...
        hv26ptGap[icent][ipt][id]=(TProfile*)inFile->Get(Form("hv26ptGap_%i_%i_%i",icent,ipt,id));
        hv28ptGap[icent][ipt][id]=(TProfile*)inFile->Get(Form("hv28ptGap_%i_%i_%i",icent,ipt,id));
        hv26pt[icent][ipt][id]=(TProfile*)inFile->Get(Form("hv26pt_%i_%i_%i",icent,ipt,id));
  ...
  for (int icent=0;icent<ncent;icent++){
    for (int ipt=0;ipt<npt;ipt++){
      for (int id=8;id<npid;id++){
        ...
        hv26ptGap[icent][ipt][id] = (TProfile*)  hv26ptGap[icent][ipt][id-8] -> Clone();
        hv28ptGap[icent][ipt][id] = (TProfile*)  hv28ptGap[icent][ipt][id-8] -> Clone();
        hv26pt[icent][ipt][id] = (TProfile*)  hv26pt[icent][ipt][id-8] -> Clone();
        ...
        hv26ptGap[icent][ipt][id] -> Add(hv26ptGap[icent][ipt][id-4]);
        hv28ptGap[icent][ipt][id] -> Add(hv28ptGap[icent][ipt][id-4]);
        hv26pt[icent][ipt][id] -> Add(hv26pt[icent][ipt][id-4]);
  ...
  for (int icent=0; icent<ncent; icent++){ // loop over centrality classes
    ...
    // 6QC, 2-sub
    term cor6Gap = term(hv26Gap[icent]);
    double v26Gap = pow(2,-1./3.)*pow(cor6Gap.mVal-9*cor2Gap.mVal*cor4Gap.mVal+12*pow(cor2Gap.mVal,3),1./6.);
    // 8QC, 2-sub
    term cor8Gap = term(hv28Gap[icent]);
    double v28Gap = pow(33,-1./8.)*pow(-cor8Gap.mVal+16*cor6Gap.mVal*cor2Gap.mVal+18*pow(cor4Gap.mVal,2)-144*cor4Gap.mVal*pow(cor2Gap.mVal,2)+144*pow(cor2Gap.mVal,4),1./8.);

    // 6QC
    term cor6 = term(hv26[icent]);
    ...
    for (int id=0;id<npid;id++){ // Differential flow calculation
      for(int ipt=0; ipt<npt; ipt++){ // loop for all pT bin
        ...
        // v26,2-sub
        term cor6redGap = term(hv26ptGap[icent][ipt][id]);
        double v26DifGap = (cor6redGap.mVal-9*cor2Gap.mVal*cor4redGap.mVal+12*cor2redGap.mVal*pow(cor2Gap.mVal,2)) / (4.0*pow(v26Gap,5));
        double ev26DifGap = 0;
        v2Dif[4][icent][id][ipt] = v26DifGap;
        v2eDif[4][icent][id][ipt] = ev26DifGap;
        // v28,2-sub
        term cor8redGap = term(hv28ptGap[icent][ipt][id]);
        double v28DifGap = (-cor8redGap.mVal+16*cor6redGap.mVal*cor2Gap.mVal+18*cor4redGap.mVal*cor4Gap.mVal-144*cor4redGap.mVal*pow(cor2Gap.mVal,2)+144*cor2redGap.mVal*pow(cor2Gap.mVal,3)) / (33.0*pow(v28Gap,7));
        double ev28DifGap = 0;
        v2Dif[5][icent][id][ipt] = v28DifGap;
        v2eDif[5][icent][id][ipt] = ev28DifGap;
        // v26
        term cor6red = term(hv26pt[icent][ipt][id]);
        double v26Dif = (cor6red.mVal-9*cor2.mVal*cor4red.mVal+12*cor2red.mVal*pow(cor2.mVal,2)) / (4.0*pow(v26,5));
        double ev26Dif = 0;
        v2Dif[6][icent][id][ipt] = v26Dif;
        v2eDif[6][icent][id][ipt] = ev26Dif;
        ...
        if (icent>=2 && icent <=4) { // 10-40%
          ...
          prV2Dif1040[4][id] -> Fill(0.5+ipt,v26DifGap,hcounter[icent][ipt][id] -> GetBinEntries(2));
          prV2Dif1040[5][id] -> Fill(0.5+ipt,v28DifGap,hcounter[icent][ipt][id] -> GetBinEntries(2));
          prV2Dif1040[6][id] -> Fill(0.5+ipt,v26Dif,hcounter[icent][ipt][id] -> GetBinEntries(1));
        }
      } // end of loop for all pT bin
  ...
  const char *grTitle[nmethod]={"v_{2}{2}","v_{2}{4}","v_{2}{2,2-sub}","v_{2}{4,2-sub}","v_{2}{6,2-sub}","v_{2}{8,2-sub}","v_{2}{6}"};
  outFile -> cd();
  for (int imeth=0; imeth<nmethod; imeth++){
    for (int id=0;id<npid;id++){
  ...
  TString strCent[4] = {"5-10%","10-20%","20-30%","30-40%"};
  for (int id=0;id<npid;id++)
  {
    ...
    for (int icent=1; icent<5; icent++)
    {
      ...
      vgrv2pt[icent-1].push_back(grDifFl[6][icent][id]);
      for (int i=0; i<nmethod; i++){
        if (i==ratioToMethod) continue;
        if (i==0) continue;
        if (i==2) continue;
        // vgrv2pt[icent-1].push_back(grDifFl[i][icent][id]);
      }
    }
    cV2PTMultPad[id] = (TCanvas*) DrawTGraph(vgrv2pt, 4,"",rangeRatio.at(0).first, rangeRatio.at(0).second, minpt, maxpt, minV2dif, maxV2dif,
                                              coordinateLeg.at(0), coordinateLeg.at(1), coordinateLeg.at(2), coordinateLeg.at(3),
  ...
  for (int icent=0;icent<ncent;icent++){
    for (int id=0;id<npid;id++){
      for (int ipt=binMinPtRFP+1;ipt<binMaxPtRFP;ipt++){
        ...
        hv26ptGap[icent][binMinPtRFP][id]           -> Add(hv26ptGap[icent][ipt][id]);
        hv28ptGap[icent][binMinPtRFP][id]           -> Add(hv28ptGap[icent][ipt][id]);
      }
    }
  }
  ...
  for (int icent=0; icent<ncent; icent++){ // loop over centrality classes
    ...
    term cor6Gap = term(hv26Gap[icent]);
    double v26Gap = pow(2,-1./3.)*pow(cor6Gap.mVal-9*cor2Gap.mVal*cor4Gap.mVal+12*pow(cor2Gap.mVal,3),1./6.);
    v2_RF[4][icent] = v26Gap;
    v2e_RF[4][icent] = 0;

    // 8QC, 2-sub
    term cor8Gap = term(hv28Gap[icent]);
    double v28Gap = pow(33,-1./8.)*pow(-cor8Gap.mVal+16*cor6Gap.mVal*cor2Gap.mVal+18*pow(cor4Gap.mVal,2)-144*cor4Gap.mVal*pow(cor2Gap.mVal,2)+144*pow(cor2Gap.mVal,4),1./8.);
    v2_RF[5][icent] = v28Gap;
    v2e_RF[5][icent] = 0;
    ...
    for (int id=0;id<npid;id++){   
      for(int ipt=binMinPtRFP; ipt<binMinPtRFP+1; ipt++){ // loop for all pT bin
        ...
        double ev24DifGap = sqrt( pow(v24Gap,-14)
            ...
            * cov44primeGap
            - 4*cor2Gap.mVal*pow(v24Gap,8)*cov2prime4primeGap));
        v2[3][id][icent] = v24DifGap;
        v2e[3][id][icent] = ev24DifGap;

        // v26,2-sub
        term cor6redGap = term(hv26ptGap[icent][ipt][id]);
        double v26DifGap = (cor6redGap.mVal-9*cor2Gap.mVal*cor4redGap.mVal+12*cor2redGap.mVal*pow(cor2Gap.mVal,2)) / (4.0*pow(v26Gap,5.));
        double ev26DifGap = 0;
        v2[4][id][ipt] = v26DifGap;
        v2e[4][id][ipt] = ev26DifGap;
        // v28,2-sub
        term cor8redGap = term(hv28ptGap[icent][ipt][id]);
        double v28DifGap = (-cor8redGap.mVal+16*cor6redGap.mVal*cor2Gap.mVal+18*cor4redGap.mVal*cor4Gap.mVal-144*cor4redGap.mVal*pow(cor2Gap.mVal,2)+144*cor2redGap.mVal*pow(cor2Gap.mVal,3)) / (33.*pow(v28Gap,7.));
        double ev28DifGap = 0;
        v2[5][id][ipt] = v28DifGap;
        v2e[5][id][ipt] = ev28DifGap;
  ...
  // vgrv2cent_chargedHardons.push_back(grRefFl[4]);
  // vgrv2cent_chargedHardons.push_back(grRefFl[5]);
  vgrv2cent_chargedHardons.push_back(grRefFl[6]);
  for (int imeth=0;imeth<nmethod;imeth++){
    if (imeth==ratioToMethod) continue;
