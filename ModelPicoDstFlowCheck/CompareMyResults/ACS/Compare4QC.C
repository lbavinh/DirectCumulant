...
void Compare4QC(){
  ...
  inputVinh[0] = new TFile("VinhPID_UrQMD_merged_ACS.root","read");
  inputVinh[1] = new TFile("VinhPID_GEANT_merged_ACS.root","read");
  inputVinh[2] = new TFile("VinhPID_Reco_merged_ACS.root","read");
  for (int i=0; i<nmethod; i++){
    for (int id=0;id<npid;id++){
      ...
      for (int icent=0;icent<5;icent++){
        ...
        grVinh[icent][i][id] -> SetTitle(Form("%s;p_{T}, GeV/c;v_{2}{4}",method[i]));
      }
    }
  }
  for (int id=0;id<npid;id++){
  ...
  for (int id=0;id<npid;id++){
    ...
    // sprintf(name,"%s, centrality 0-10%%",pidFancyNames.at(id).Data());
    ...
    // sprintf(name,"%s_Cent0-10.png",pidNames.at(id).Data());
