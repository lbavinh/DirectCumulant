...
vector<TGraphErrors*> PlotV2IntegratedChargedHadrons(TString model, TString energy)
{
  TString inputFirstRunFileName = Form("../FirstRun_%s_%s.root",model.Data(),energy.Data());
  TString inputSecondRunFileName = Form("../SecondRun_%s_%s.root",model.Data(),energy.Data());
  TString inputFHCalEventPlane = Form("../FHCal_%s_%s_1.root",model.Data(),energy.Data());
  Double_t maxpt = 3.6;    // max pt for differential flow
  Double_t minpt = 0.;     // min pt for differential flow
  Double_t maxptRF = 3.0;   // max pt for reference flow
  ...
  bool bUseProduct = 1;
  Int_t nmethod = 10;
  TString title[]={"v_{2}{#Psi_{2,TPC}}","v_{2}{SP}","v_{2}{2}","v_{2}{4}","v_{2}{6}","v_{2}{8}","v_{2}{LYZ, Sum}","v_{2}{LYZ, Prod.}","v_{2}{2,#eta-gap}","v_{2}{#Psi_{1,FHCal}}"};
  const int markerStyle[] = {24,22,27,21,20,25,28,26,23,30};
  ...
  TFile *fiFHCal = new TFile(inputFHCalEventPlane.Data(),"read");
  ...
  TProfile *prV2FHCalEPInt = PlotV2FHCalEPIntegrated(prV2FHCalEP3D,minptRF,maxptRF,eta_cut);
  gr[9] = Converter(prV2FHCalEPInt);
