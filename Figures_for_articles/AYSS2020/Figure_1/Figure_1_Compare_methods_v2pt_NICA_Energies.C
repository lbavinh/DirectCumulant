void Figure_1_Compare_methods_v2pt_NICA_Energies(){
  ...
  const float maxV2 = 0.12;
  const float minV2 = -0.01;
  const float minV2Ratio = 0.7;
  const float maxV2Ratio = 1.3;
  const float leg_coordinate[4]={0.7,0.05,0.85,0.32}; // top left: 0.28,0.7,0.6,0.99
  ...
  TString xAxisName = {"#font[132]{#it{p_{T}} (GeV/c)}"};
  TString ratioName[nmethod-1]={"v2FHCal/v22","v24/v22","v2TPC/v22","v2SP/v22"};
  // TString legendEntries[nmethod]={"v_{2}{#Psi_{1,FHCal}}","v_{2}{4}","v_{2}{#Psi_{2,TPC}^{}}","v_{2}^{SP}{Q_{2,TPC}}","v_{2}{2}"};
  TString legendEntries[nmethod]={"#it{v}_{2}{#Psi_{1,FHCal}}","#it{v}_{2}{4}","#it{v}_{2}{#Psi_{2,TPC}^{}}","#it{v}_{2}^{SP}{Q_{2,TPC}}","#it{v}_{2}{2}"};
  // TString pidInPads[]={"#sqrt{s_{NN}} = 4.5 GeV","#sqrt{s_{NN}} = 7.7 GeV","#sqrt{s_{NN}} = 11.5 GeV"};
  TString pidInPads[]={"#it{#sqrt{s_{NN}}} = 4.5 GeV","#it{#sqrt{s_{NN}}} = 7.7 GeV","#it{#sqrt{s_{NN}}} = 11.5 GeV"};
  TFile *input[nenergy][2];
  TFile *inputFlowANA[nenergy]; 
  TFile *inputScalarProduct[nenergy];
  ...
  for (int ien=0;ien<nenergy;ien++){
    ...
    inputScalarProduct[ien] = new TFile(Form("v2_UrQMD_%sGeV_V2SP.root",energy[ien].Data()),"read");
    inputFlowANA[ien] = new TFile(Form("v2_UrQMD_%sGeV_V2R1.root",energy[ien].Data()),"read");
    for (int id=0;id<npid;id++){
      grV2[ien][id][3] = (TGraphErrors*)inputScalarProduct[ien]->Get(Form("gr_rebinned_cent_10_40_pv2TPC_pid%i",id)); // v2SP
      grV2[ien][id][0] = (TGraphErrors*)inputFlowANA[ien]->Get(Form("grv22fhcal_%i",id)); // v2FHCal
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int id=0;id<npid;id++){
      for (int imeth=0;imeth<nmethod;imeth++){
        if (!grV2[ien][id][imeth]) cout << "Empty " << ien <<", "<<id<<", "<<imeth<<endl;
      }
    }
  }
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int id=0;id<npid;id++){
      ...
      grV2[ien][id][0] -> SetMarkerColor(kBlue+2);
      grV2[ien][id][0] -> SetLineColor(kBlue+2);
      grV2[ien][id][0] -> SetMarkerStyle(kOpenTriangleUp);

      grV2[ien][id][1] -> SetMarkerColor(kRed+2);
      grV2[ien][id][1] -> SetLineColor(kRed+2);
      grV2[ien][id][1] -> SetMarkerStyle(kFullCircle);  

      grV2[ien][id][2] -> SetMarkerColor(kGreen+3);
      grV2[ien][id][2] -> SetLineColor(kGreen+3);
      grV2[ien][id][2] -> SetMarkerStyle(kFullCross);

      grV2[ien][id][3] -> SetMarkerColor(kYellow+2);
      grV2[ien][id][3] -> SetLineColor(kYellow+2);
      grV2[ien][id][3] -> SetMarkerStyle(kOpenTriangleDown);

      grV2[ien][id][4] -> SetMarkerColor(kGray+3);
      grV2[ien][id][4] -> SetLineColor(kGray+3);
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int id=0;id<npid;id++){
      ...
      grRatioV2[ien][id][0] -> SetMarkerColor(kBlue+2);
      grRatioV2[ien][id][0] -> SetLineColor(kBlue+2);
      grRatioV2[ien][id][0] -> SetMarkerStyle(kOpenTriangleUp);

      grRatioV2[ien][id][1] -> SetMarkerColor(kRed+2);
      grRatioV2[ien][id][1] -> SetLineColor(kRed+2);
      grRatioV2[ien][id][1] -> SetMarkerStyle(kFullCircle);

      grRatioV2[ien][id][2] -> SetMarkerColor(kGreen+3);
      grRatioV2[ien][id][2] -> SetLineColor(kGreen+3);
      grRatioV2[ien][id][2] -> SetMarkerStyle(kFullCross);

      grRatioV2[ien][id][3] -> SetMarkerColor(kYellow+2);
      grRatioV2[ien][id][3] -> SetLineColor(kYellow+2);
      grRatioV2[ien][id][3] -> SetMarkerStyle(kOpenTriangleDown);

      grRatioV2[ien][id][4] -> SetMarkerColor(kGray+3);
      grRatioV2[ien][id][4] -> SetLineColor(kGray+3);
  ...
  TPaveLabel* title = new TPaveLabel(0.1,0.945,0.9,0.965,"UrQMD, Au+Au, 10-40%, Ch. hadrons");
  ...
  for (int ipad=0;ipad<6;ipad++){
    can->cd(ipad+1);
    if (ipad==0) h[ipad] = new TH2F(Form("pad_%i",ipad+1),Form(";%s;#font[132]{#it{v}_{2}}",xAxisName.Data()),1,minpt,maxpt,1,minV2,maxV2);
    if (ipad==1 || ipad==2) h[ipad] = new TH2F(Form("pad_%i",ipad+1),"",1,minpt,maxpt,1,minV2,maxV2);
    if (ipad==3) h[ipad] = new TH2F(Form("pad_%i",ipad+1),";;#font[132]{Ratio to #it{v}_{2}{2}}",1,minpt,maxpt,1,minV2Ratio,maxV2Ratio);
    ...
    tex1.DrawLatex(0.3,maxV2*0.9,Form("%s",pidInPads[ipad].Data()));
    if (ipad==0) {
    ...
    else
    {
      for (int imeth=0; imeth<nmethod; imeth++)
      {
        grRatioV2[ipad-3][0][imeth]->Draw("P");
      }
    }
  ...
  // can->SaveAs(Form("Figure_1_Compare_methods_v2pt_NICA_Energies.eps"));
  can->SaveAs(Form("Figure_1_Compare_methods_v2pt_NICA_Energies.pdf"));
  // can->SaveAs(Form("Figure_1_Compare_methods_v2pt_NICA_Energies.C"));
  gROOT->SetStyle("Pub");
  can->SaveAs(Form("Figure_1_Compare_methods_v2pt_NICA_Energies.png"));
  }
  if (drawEnergy==2)
  {
  TCanvas *can = new TCanvas("can","",200,10,1600,1000);
  can->SetLeftMargin(0.15);
  can->SetRightMargin(0.01);
  can->SetBottomMargin(0.2);
  can->SetTopMargin(0.2);
  TPaveLabel* title = new TPaveLabel(0.1,0.945,0.9,0.965,"UrQMD, Au+Au, 10-40%, charged hadrons");
  title->SetBorderSize(0);
  title->SetFillColor(0);
  title->SetTextFont(textFont);
  title->SetTextSize(2.5);
  ...
  for (int ipad=0;ipad<4;ipad++){
    ...
    if (ipad==2) h[ipad] = new TH2F(Form("pad_%i",ipad+1),";;#font[132]{Ratio to #it{v}_{2}{2}}",1,minpt,maxpt,1,minV2Ratio,maxV2Ratio);
    ...
    tex1.SetTextAlign(13);
    tex1.SetTextSize(labelSizeUpperPlot+0.02);
    tex1.DrawLatex(0.3,maxV2*0.9,Form("%s",pidInPads[ipad+1].Data()));
    if (ipad==0) {
    ...
    if (ipad==1) {
      // tex.DrawLatex(maxpt*0.9,maxV2*0.9,Form("UrQMD                             "));
      TLegend *leg_pt = new TLegend(leg_coordinate[0]+0.05,leg_coordinate[1],leg_coordinate[2]+0.05,leg_coordinate[3]);
      leg_pt->SetBorderSize(0);
      leg_pt->SetTextSize(labelSizeUpperPlot);
      leg_pt->SetTextFont(textFont);
      leg_pt->AddEntry(grV2[0][0][1],legendEntries[1].Data(),"p");
    ...
    else
    {
      for (int imeth=0; imeth<nmethod; imeth++)
      {
        grRatioV2[ipad-1][0][imeth]->Draw("P");
      }
    }
  ...
  // can->SaveAs(Form("Figure_1_Compare_methods_v2pt_NICA_Energies.eps"));
  can->SaveAs(Form("Figure_1_Compare_methods_v2pt_NICA_Energies_2energy.pdf"));
  // can->SaveAs(Form("Figure_1_Compare_methods_v2pt_NICA_Energies.C"));
  gROOT->SetStyle("Pub");
  can->SaveAs(Form("Figure_1_Compare_methods_v2pt_NICA_Energies_2energy.png"));    
  }
}