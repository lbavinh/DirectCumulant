...
TString input2  = Form("../SecondRun_AMPT15_%s.root",energy.Data());
void PlotV2DifferentialChargedHadrons(TString inputFirstRunFileName = input1, TString inputSecondRunFileName = input2)
{
  const vector<pair<int,int>> centrality = {{0,5},{5,10},{10,15},{15,20},{20,25},{25,30},{30,35},{35,40},{40,45},{45,50},{50,55}};
  const TString padName[]={"(a)","(b)","(c)","(d)","(e)","(f)","(g)","(h)","(i)","(j)","(k)","(l)"};
  ...
  Double_t eta_cut = 2.5;  // pseudorapidity acceptance window for flow measurements
  ...
  const int numberOfCentToDraw = 6;
  ...
  can->SaveAs(Form("CompareMethods_Diff_v2{4}_vs_v2{LYZ,Prod}_at_%s_5_centrality_bin.png",energy.Data()));
  can->SaveAs(Form("CompareMethods_Diff_v2{4}_vs_v2{LYZ,Prod}_at_%s_5_centrality_bin.pdf",energy.Data()));
