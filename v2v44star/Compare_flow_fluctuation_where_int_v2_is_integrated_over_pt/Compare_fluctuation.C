void Compare_fluctuation(){
  ...
  const TString leg_header[2] = {"UrQMD,10-40%","SMASH,10-40%"};
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int imod=0;imod<nmodel;imod++){
      ...
      for (int imeth=0;imeth<nmethod;imeth++){ // v24 = 1; v22(eta-gap) = 3
        grV2[ien][imod][imeth] = (TGraphErrors*)input[ien][imod]->Get(Form("gr_cent10-40_%i_0",imeth)); // "_0" - Charged Hadrons
      }
    }
  }
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int imod=0;imod<nmodel;imod++){
      ...
      grRatioV2[ien][imod] -> SetTitle(Form("%s GeV;p_{T} [GeV/c];v_{2}{4}/v_{2}{2}",energy[ien].Data()));
  ...
  can->Divide(1,2,0,0);
  can->cd(1);
  TH2F *h1 = new TH2F("h1",";p_{T} [GeV/c];v_{2}{4}/v_{2}{#eta-sub}",1,minpt,maxpt,1,minV2Ratio,maxV2Ratio);
  ...
  can->cd(2);
  TH2F *h2 = new TH2F("h2",";p_{T} [GeV/c];v_{2}{4}/v_{2}{#eta-sub}",1,minpt,maxpt,1,minV2Ratio,maxV2Ratio);
  ...
  // leg_pt2->Draw();
  lineOne.DrawLine(minpt,1.,maxpt,1.);
  can->SaveAs("v24_v2EP_ratiov2pt.png");
