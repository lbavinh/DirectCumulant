...
// Flags
bool saveAsPNG = true;
int ratioToMethod = 1; // v22,v24,v22eta-sub,v24eta-gap
...
const int binMaxPtRFP = 9; // 2.8 GeV
...
void PlotV2QCumulant_with2subs(){

  TFile *outFile = new TFile(Form("./v2_UrQMD_7.7GeV_Model.root"),"recreate");
  TString outDirName=(TString)Form("UrQMD_7.7_Model");
  ...
  for (int icent=0; icent<ncent; icent++){ // loop over centrality classes
    ...
    for (int id=0;id<npid;id++){ // Differential flow calculation
      for(int ipt=0; ipt<npt; ipt++){ // loop for all pT bin
        ...
        if (id == 8)
        {
          prV2Ref[0]->Fill(icent,v22Dif,hcounter[icent][ipt][id] -> GetBinEntries(1));
          prV2Ref[1]->Fill(icent,v24Dif,hcounter[icent][ipt][id] -> GetBinEntries(1));
          prV2Ref[2]->Fill(icent,v22DifGap,hcounter[icent][ipt][id] -> GetBinEntries(2));
          prV2Ref[3]->Fill(icent,v24DifGap,hcounter[icent][ipt][id] -> GetBinEntries(2));
        }
  ...
  for (int imeth=0; imeth<nmethod; imeth++){
    for (int id=0;id<npid;id++){
      for(int ipt=0; ipt<npt; ipt++){
        ...
        // if (id==8 && imeth==2) std::cout << v2Dif1040[imeth][id][ipt] << std::endl;
  ...
  for (int icent=0; icent<ncent; icent++){ // loop over centrality classes
    ...
    std::cout << "v22 = " << v2_RF[0][icent] << "\t" << prV2Ref[0]->GetBinContent(icent+1) << "\t v24 = " << v2_RF[1][icent] << "\t" << prV2Ref[1]->GetBinContent(icent+1) << std::endl;
