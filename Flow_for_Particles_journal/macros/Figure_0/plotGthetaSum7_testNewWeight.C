...
void plotGthetaSum7_testNewWeight()
{
  TFile *fi = new TFile("../FirstRun_7.7_new.root","read");
  const int ncent = 9;
  const int rbins = 500;
  ...
  tex.DrawLatex(55,0.01,"#splitline{AMPT SM, #it{#sigma}_{p}=1.5 mb}{Au+Au at #sqrt{#it{s}_{NN}} = 7.7 GeV}");
  
  gPad->SetLogy();
  gStyle->SetOptStat(0);
  c.SaveAs("Gtheta77_testNewWeight.pdf");
  c.SaveAs("Gtheta77_testNewWeight.png");
}
