...
void ComparePure(){
  ...
  inputDim = new TFile("Vinh_multilt0_withoutptweighted.root","read");
  for (int i=0; i<3; i++){
    for (int icent=0; icent<ncent; icent++){
      sprintf(name,"gr_%i_%i",icent,i);
      grDim[icent][i] = (TGraphErrors*)inputDim->Get(name);
      grDim[icent][i] -> SetTitle("Mult>0; p_{T}, GeV/c; v_{2}");
  ...
  inputVinh = new TFile("Vinh_multilt5_withoutptweighted.root","read");
  for (int i=0; i<3; i++){
    for (int icent=0; icent<ncent; icent++){
      sprintf(name,"gr_cent%i_%i",icent,i);
      grVinh[icent][i] = (TGraphErrors*)inputVinh->Get(name);
      grVinh[icent][i] -> SetTitle("Mult>5; p_{T}, GeV/c; v_{2}");
