...
void v2plot(TString model ="UrQMD", TString energy = "7.7GeV") {
  ...
  TString inFileName = (TString) Form("../ROOTFile/%s_%s_vsBimp.root",model.Data(),energy.Data());
  TFile *outFile = new TFile(Form("./v2_%s_%s_vsBimp.root",model.Data(),energy.Data()),"recreate");
  TString outDirName = (TString)Form("%s_%s_vsBimp",model.Data(),energy.Data());
  ...
  int drawDifferentialFlowTill = 0; // Draw v2 vs pT (10% b cut) till: 0: no drawing; 1: till 10%; 2: till 20%; etc.
  // Constants
  ...
  const double bin_cent[ncent] = {0.5,1.5,2.5,3.5,4.5,5.5,6.5,7.5,8.5,9.5,10.5,11.5,12.5,13.5,14.5,15.5,16.5,17.5,18.5,19.5};
  const double bin_centE[ncent] = {0};
  const float eta_gap = 0.05;

  const double mincent = 0.;  // for v2 vs b
  const double maxcent = 20.; // for v2 vs b

  const double minV2int = -0.005; // for v2 vs b plotting
  const double maxV2int = 0.1; // for v2 vs b plotting
  const double minV2dif = -0.005; // for v2 vs pt plotting
  const double maxV2dif = 0.1; // for v2 vs pt plotting

  std::vector<TString> pidNames = {"hadron_pos", "pion_pos", "kaon_pos", "proton", "hadron_neg", "pion_neg", "kaon_neg", "proton_anti", "pion", "kaon"};
  ...
  for (int icent=0; icent<ncent; icent++){ // loop over b classes
  ...
  */
  ...
  const char *grTitleDF[nmethod]={"v_{2}{2};b (fm);v_{2}",
                                  "v_{2}{4};b (fm);v_{2}",
                                  "v_{2}{2,#eta-gap};b (fm);v_{2}"};
  outFile -> cd();
  ...
  for (int id=0;id<npid;id++){
    ...
    if (saveAsPNG) cV2PT[id] -> SaveAs(Form("./%s/%s.png",outDirName.Data(),pidNames.at(id).Data()));
  }
  ...
  std::vector<TGraphErrors*> vgrv2centRF;
  for (int imeth=0; imeth<nmethod; imeth++){
    // grRefFl[imeth][id] -> SetTitle(Form("V2 vs. pT, %s, b 10-40%%, %s",pidNames.at(id).Data(),grTitleDF[imeth]));
    grRefFl[imeth] -> SetTitle(grTitleDF[imeth]);
    grRefFl[imeth] -> Write(Form("grRF_%i",imeth));
  }
  vgrv2centRF.push_back(grRefFl[2]);
  for (int imeth=0;imeth<nmethod-1;imeth++){
    if (imeth==excludeMethod) continue;
    vgrv2centRF.push_back(grRefFl[imeth]);
  }
  cV2CentRF = (TCanvas*) DrawTGraph(vgrv2centRF,"",rangeRatio.at(0).first, rangeRatio.at(0).second, mincent, maxcent, minV2dif, maxV2dif,
                                        coordinateLeg.at(0), coordinateLeg.at(1), coordinateLeg.at(2), coordinateLeg.at(3),
                                        level.Data(), "Reference flow");
  cV2CentRF -> SetName(hname);
  if (saveAsPNG) cV2CentRF -> SaveAs(Form("./%s/hadron.png",outDirName.Data()));
