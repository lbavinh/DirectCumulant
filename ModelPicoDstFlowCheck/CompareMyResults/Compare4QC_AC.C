  ...
  for (int i=0; i<nmethod; i++){
    for (int id=0;id<npid;id++){
      for (int icent=0;icent<ncent-2;icent++){
        grVinh[icent][i][id] = (TGraphErrors*)inputVinh[i]->Get(Form("gr_cent%i_1_%i",icent,id));
        grVinh[icent][i][id] -> SetTitle(Form("%s;p_{T}, GeV/c;v_{2}{4}",method[i].Data()));
      }
      grVinh[5][i][id] = (TGraphErrors*)inputVinh[i]->Get(Form("gr_cent10-40_1_%i",id));
      grVinh[5][i][id] -> SetTitle(Form("%s;p_{T}, GeV/c;v_{2}{4}",method[i].Data()));
      grVinh[6][i][id] = (TGraphErrors*)inputVinh[i]->Get(Form("grRF_1_%i",id));
      grVinh[6][i][id] -> SetTitle(Form("%s;cent, %%;v_{2}{4}",method[i].Data()));      
    }
  }
  for (int id=0;id<npid;id++){
