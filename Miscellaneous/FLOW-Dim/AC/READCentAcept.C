...
void read(const char *infile, const char *savefile = "~/GIT/Acept.root")
{
  ...
  double dCOSp1[Nb][NN];
  double dSINp1[Nb][NN];
  double dCOSp1f2[Nb][NN];
  double dSINp1f2[Nb][NN];
  double dCOSp1f2mf3[Nb][NN];
  double dSINp1f2mf3[Nb][NN];
  double dCOSp1mf2mf3[Nb][NN];
  double dSINp1mf2mf3[Nb][NN];
  ...
  for (int k = 0; k < Nb; k++)
  {
    ...
    IMPsv2[k] = sv2[k] - (pow(COSf1[k]->GetMean(), 2) + pow(SINf1[k]->GetMean(), 2));
    IMPv2[k] = pow(fabs(IMPsv2[k]), 0.5); // v22int[icent]
    ...
    for (int m = 0; m < NN; m++)
    {
      ...
      IMPdn2[k][m] = DiffSV2[k][m]->GetMean() - dCOSp1[k][m] * dCOSf1[k] - dSINp1[k][m] * dSINf1[k];
      IMPdv2[k][m] = IMPdn2[k][m] / pow(fabs(IMPsv2[k]), 0.5);
    ...
    for (int m = 0; m < NN; m++)
    {
      ...
      // (C.46)
      Rv4[k][m] = (pow(fabs(cn4[k]), -3.5)) 
      * (pow((2 * sv2[k] * sv2[k] * dn2[k][m] - 3 * sv2[k] * dsv4[k][m] + 2 * sv4[k] * dn2[k][m]), 2) * sigmaX(SV2[k], 1) +
  ...
  for (int kk = 0; kk < Nb; kk++)
  {
    ...
    IntV2[kk] = v2[kk];
    ErV2[kk] = pow(sigmaX(SV2[kk], 1), 0.5) / (2 * pow(sv2[kk], 0.5)); // (C.24)
    ...
    grv4IMP[kk]->SetName(name);
    grv4IMP[kk]->GetYaxis()->SetRangeUser(-0.01, 0.26);
    ...
    grv2IMP[kk]->SetTitle("v_{2}{2} acept");
    grv2IMP[kk]->Write();
  ...
  for (int k = 0; k < 6; k++)
  {
    ...
    legdif[k]->AddEntry(grv4IMP[k], "V_{2}{4}", "pe");
    legdif[k]->AddEntry(grv2IMP[k], "V_{2}{2}", "pe");
  ...
  TLegend *leg2 = new TLegend(0.1, 0.55, 0.3, 0.9);
  ...
  grIIMPv4->SetTitle("");
  grIIMPv4->GetXaxis()->SetTitle("Cent, %");
  grIIMPv4->GetYaxis()->SetTitle("V_{2}");
  ...
  TLegend *leg3 = new TLegend(0.1, 0.75, 0.29, 0.95);
  ...
  dleg->AddEntry(grv4IMP[cent], "V_{2}{4}", "pe");
  dleg->AddEntry(grv2IMP[cent], "V_{2}{2}", "pe");
