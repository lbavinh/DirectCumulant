#define PLOTV2ETASUBEP
// bool bDebug = false;
TProfile *PlotV2FHCalEPIntegrated(TProfile3D* &prV2,
...
TProfile *PlotV2FHCalEPDifferentialVersusPt(TProfile3D *const &prV2,
...
TProfile *PlotV2FHCalEPDifferentialVersusEta(TProfile3D*const &prV2,
                           const double pt_low = 0.2,
                           const double pt_high = 3.0,
                           const double cent_low = 10,
                           const double cent_high = 40)
...
void PlotV2FHCalEventPlane(TString inputFileName = "SecondRun.root")
{
  TFile *fi = TFile::Open(inputFileName.Data());
  TProfile3D *pr = (TProfile3D *)fi->Get("prV2FHCalEventPlane");
  ...
  TProfile *prV2diffpt = PlotV2FHCalEPDifferentialVersusPt(pr,10,40,1.5);
  ...
  TProfile *prV2diffpt4080 = PlotV2FHCalEPDifferentialVersusPt(pr,40,80,1.5);
  ...
  TProfile *prV2int = PlotV2FHCalEPIntegrated(pr,0.2,3.5,1.5);
  ...
  TProfile *prV2diffEta = PlotV2FHCalEPDifferentialVersusEta(prClone, 0.2, 3.0, 10, 40);
