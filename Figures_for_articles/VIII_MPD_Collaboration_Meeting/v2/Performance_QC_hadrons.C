...
// #include "constants.C"
// vector<TGraphErrors*>*
void Performance_QC_hadrons(TString inputModel = "FirstRun_vHLLEUrQMD_11.5_Model.root", TString inputRECO = "FirstRun_vHLLEUrQMD_11.5_Reco_Nhits_32.root")
{
...
const Double_t pTBin[npt + 1] = {0., 0.2, 0.4, 0.6, 0.8, 1., 1.2, 1.4, 1.6, 1.8, 2.0, 2.2, 2.4, 2.6, 2.8, 3.2, 3.6};
  ...
  {
    graph[i][0][0] = PlotV2QCumulant_Model(inputModel,3,firstCentToDraw+i,8); // pi
    graph[i][0][1] = PlotV2QCumulant(inputRECO,2,firstCentToDraw+i,8);
    // graph[i][1][0] = PlotV2QCumulant_Model(inputModel,3,firstCentToDraw+i,8); // p
    // graph[i][1][1] = PlotV2QCumulant(inputRECO,2,firstCentToDraw+i,8);
  }
  ...
  c->SaveAs("Performance_2QC_hadrons.pdf");
  c->SaveAs("Performance_2QC_hadrons.png");
