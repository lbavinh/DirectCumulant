...
const Int_t sum = (h1<0?-1*h1:h1)+(h2<0?-1*h2:h2)+(h3<0?-1*h3:h3)+(h4<0?-1*h4:h4) // ? : conditional operator: sum=(h1<0?-1*h1:h1) is equivalent with if(h1<0) {sum = -1*h1;} else {sum=h1;}
...
const Int_t maxPower = maxCorrelator+1; // indicies for loops start at 0, so we need real maxPower = maxCorrelator + 1 (maybe)
...
void Cosmetics()
{
 ...
 for(Int_t cs=0;cs<2;cs++) 
 {
  for(Int_t c=0;c<maxCorrelator;c++)
  {
   ...
   correlations[cs][c]->Sumw2(); // Create structure to store sum of squares of weights. 
...
void CalculateQvectors()
{
 ...
 Double_t wPhiToPowerP = 1.; // particle weight raised to power p - возводить в p степень
 for(Int_t i=0;i<nParticles;i++) // loop over particles
 {
  ...
  for(Int_t h=0;h<maxHarmonic;h++)
  {
   for(Int_t p=0;p<maxPower;p++) // p=0,1,2,3,4,5,6,7,8
   {
    if(bUseWeights){wPhiToPowerP = pow(wPhi,p);}
    Qvector[h][p] += TComplex(wPhiToPowerP*TMath::Cos(h*dPhi),wPhiToPowerP*TMath::Sin(h*dPhi)); // This is formula B1 in the paper
   } //  for(Int_t p=0;p<maxPower;p++)
  } // for(Int_t h=0;h<maxHarmonic;h++)
 } //  for(Int_t i=0;i<nParticles;i++) // loop over particles
...
TComplex Q(Int_t n, Int_t p) // n-order of harmonic, p-order of particle correlation (chắc là như vậy)
{
 // Using the fact that Q{-n,p} = Q{n,p}^*. 
 
 if(n>=0){return Qvector[n][p];}      //  TComplex Qvector[maxHarmonic][maxPower];
 return TComplex::Conjugate(Qvector[-n][p]); // conjugate - сопряженное, trường hợp thông số truyền cho hàm Q với n<0 thì lấy phần tử  Qvector[-n][p] rồi tiến hành phép đối số  phức 
...
void TwoNestedLoops(Int_t n1,Int_t n2)
{
 ...
 for(Int_t i1=0;i1<nParticles;i1++)
 {
  ...
  for(Int_t i2=0;i2<nParticles;i2++)
  {
   ...
   // Fill profiles:
   nestedLoops[0][0]->Fill(0.5,TMath::Cos(n1*phi1+n2*phi2),w1*w2); // Fill(x,y,w)
