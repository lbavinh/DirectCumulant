...
TString input1 = Form("../FirstRun_AMPT08_%s_new.root",energy.Data());
TString input2  = Form("../SecondRun_AMPT08_%s_new.root",energy.Data());
void PlotV2DifferentialChargedHadrons_testNewWeight_AMPT08_forPresentation(TString inputFirstRunFileName = input1, TString inputSecondRunFileName = input2)
{
  ...
  const int markerStyle[] = {24,22,23,27,25,28,20};
  ...
  const double maxRatio = 1.21;
  const double minRatio = 0.79;
  const double maxX = 2.65;
  ...
  for (int ic = 0; ic < ncent; ic++)
  {
    ...
    {    
      ...
      if (ic==3) cout << v2diffPro[ic][ipt] << endl;
    }
  ...
  TPaveLabel* label = new TPaveLabel(0.1,0.95,0.9,0.99,Form("AMPT SM, #it{#sigma}_{p}=0.8 mb, Au+Au at #sqrt{s_{NN}} = %s GeV, Charged Hadrons",energy.Data()));
  ...
  for (int ic=firstCentToDraw; ic<firstCentToDraw+numberOfCentToDraw; ic++)
  {
    ...
    tex.DrawLatex(0.2,maxRatio*0.98,padName[padID-1].Data());
  }
  can->SaveAs(Form("CompareMethods_Diff_v2{4}_vs_v2{LYZ,Prod}_at_%s_testNewWeight_AMPT08_forPresentation.png",energy.Data()));
  can->SaveAs(Form("CompareMethods_Diff_v2{4}_vs_v2{LYZ,Prod}_at_%s_testNewWeight_AMPT08_forPresentation.pdf",energy.Data()));
