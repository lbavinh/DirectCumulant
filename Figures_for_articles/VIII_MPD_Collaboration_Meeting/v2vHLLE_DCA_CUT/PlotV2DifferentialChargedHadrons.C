...
const TString energy = "vHLLEUrQMD_11.5_Model";
// const TString energy = "vHLLEUrQMD_11.5_Reco_Nhits_16_DCA_2sigma";
...
void PlotV2DifferentialChargedHadrons(TString inputFirstRunFileName = input1, TString inputSecondRunFileName = input2)
{
  ...
  auto *prV2EP3D = (TProfile2D*) firun2->Get("ETASUBEP/prV2EtaSubvsPt_pid8");
  auto *prV2SP3D = (TProfile2D*) firun2->Get("SP/prV2SPvsPt_pid8");
  for (int i = 0; i< ncent; i++)
  {
    TProfile *prV2EPInt = PlotV2vsPt(prV2EP3D,bin_cent[i],bin_cent[i+1]-1);
    TProfile *prV2SPInt = PlotV2vsPt(prV2SP3D,bin_cent[i],bin_cent[i+1]-1);
  ...
  for (int icent=0; icent<ncent; icent++){ // loop over centrality classes
    hv22[icent] = (TProfile*)firun1->FindObjectAny(Form("hv22_%i",icent));
    hv24[icent] = (TProfile*)firun1->FindObjectAny(Form("hv24_%i",icent));
    hcov24[icent] = (TProfile*)firun1->FindObjectAny(Form("hcov24_%i",icent));
    hv22Gap[icent] = (TProfile*)firun1->FindObjectAny(Form("hv22Gap_%i",icent));
    for(int ipt=0; ipt<npt; ipt++){ // loop over pt bin
      for (int id=0;id<npid;id++){
        hv22pt[icent][ipt][id]=(TProfile*)firun1->FindObjectAny(Form("hv22pt_%i_%i_%i",icent,ipt,id));
        hv24pt[icent][ipt][id]=(TProfile*)firun1->FindObjectAny(Form("hv24pt_%i_%i_%i",icent,ipt,id));
        hcov22prime[icent][ipt][id]=(TProfile*)firun1->FindObjectAny(Form("hcov22prime_%i_%i_%i",icent,ipt,id));
        hcov24prime[icent][ipt][id]=(TProfile*)firun1->FindObjectAny(Form("hcov24prime_%i_%i_%i",icent,ipt,id));
        hcov42prime[icent][ipt][id]=(TProfile*)firun1->FindObjectAny(Form("hcov42prime_%i_%i_%i",icent,ipt,id));
        hcov44prime[icent][ipt][id]=(TProfile*)firun1->FindObjectAny(Form("hcov44prime_%i_%i_%i",icent,ipt,id));
        hcov2prime4prime[icent][ipt][id]=(TProfile*)firun1->FindObjectAny(Form("hcov2prime4prime_%i_%i_%i",icent,ipt,id));
        hv22ptGap[icent][ipt][id]=(TProfile*)firun1->FindObjectAny(Form("hv22ptGap_%i_%i_%i",icent,ipt,id));
        hcov22primeGap[icent][ipt][id]=(TProfile*)firun1->FindObjectAny(Form("hcov22primeGap_%i_%i_%i",icent,ipt,id));
      }
  ...
  can->SaveAs(Form("V2pT_vHLLE_at_%s.png",energy.Data()));
  can->SaveAs(Form("V2pT_vHLLE_at_%s.pdf",energy.Data()));
