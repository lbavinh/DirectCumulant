...
void CalStatErrCent1040(TString inFileName,double v2eDif1040[nmethod][npid][npt]){
  ...
  for (int icent=0; icent<5; icent++){ // loop over centrality classes
...
TGraphErrors* PlotV2QCumulantCentrality1040(TString inputFileName, Int_t METH=2, Int_t ID=8){

  // Flags
  bool saveAsPNG = false;
  bool saveGraph = true;
  int ratioToMethod = 2; // not including i-th method in v2 plotting, where i=0,1,2,3 correspond v22,v24,v2eta-sub,v22eta-gap, respectively
  ...
  // const int binMaxPtRFP = 17; // 2.8 GeV
  ...
  TFile *outFile;
  if (saveGraph) outFile = new TFile(Form("./v2_%s.root",inputFileName.Data()),"recreate");
  ...
  for (int imeth=0; imeth<nmethod; imeth++){
    for (int id=0;id<npid;id++){
      ...
      if (saveGraph) grDifFl1040[imeth][id] -> Write(Form("gr_cent10-40_%i_%i",imeth,id));
    }
  }
  ...
  for (int id=0;id<npid;id++){
    ...
    cV2PT1040[id] = (TCanvas*) DrawTGraph(vgrv2pt1040,"",rangeRatio.at(0).first, rangeRatio.at(0).second, minpt, maxpt, minV2dif, maxV2dif,
                                          ...
                                          level.Data(), hname, true, Form("Ratio to %s",grTitle[ratioToMethod]));
