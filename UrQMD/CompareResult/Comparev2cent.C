...
void Comparev2cent(){
  ...
  for (int i=0; i<3; i++){
    ...
    grDim[i] -> SetTitle("Dim; cent, %; v_{2}");
  ...
  inputVinh = new TFile("Vinh_RF.root","read");
  for (int i=0; i<3; i++){

      sprintf(name,"grRF_%i",i);
      grVinh[i] = (TGraphErrors*)inputVinh->Get(name);
      grVinh[i] -> SetTitle("Vinh; cent, %; v_{2}");
  ...
  for (int i=0; i<3; i++){
      sprintf(name,"");
      can[i] = (TCanvas*) DrawTGraph(grDim[i],grVinh[i],ch[i],0.69,1.31,0.,80,0,0.25,0.18,0.72,0.42,0.89,name);
