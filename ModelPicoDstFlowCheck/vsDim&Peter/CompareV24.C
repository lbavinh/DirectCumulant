  ...
  for (int i=0; i<nmethod; i++){
    for (int id=0;id<npid;id++){
      ...
      for (int icent=0;icent<ncent;icent++){
        ...
        grVinh[icent][i][id] -> SetTitle("[1] Vinh;p_{T}, GeV/c;v_{2}{4}");
      }
    }
  }
  ...
  for (int i=0;i<nmethod;i++){
    for (int icent=0;icent<ncent;icent++){
      for (int id=0;id<4;id++){
        grDim[icent][i][id] = (TGraphErrors*)inputDim[i][0]->Get(Form("gr_cent%i_2",icent));
        grDim[icent][i][id] -> SetTitle("[2] Dim;p_{T}, GeV/c;v_{2}{4}");
      }
      for (int id=4;id<npid;id++){
        grDim[icent][i][id] = (TGraphErrors*)inputDim[i][1]->Get(Form("gr_cent%i_2",icent));
        grDim[icent][i][id] -> SetTitle("[2] Dim;p_{T}, GeV/c;v_{2}{4}");
      }
    }
  }
  ...
  for (int i=0; i<nmethod; i++){
    for (int id=0;id<npid;id++){
      for (int icent=0;icent<ncent;icent++){
        ...
        sprintf(name,"v24%s_%s_Cent%i-%i.png",pidNames.at(id).Data(),method[i],icent*10,(icent+1)*10);
