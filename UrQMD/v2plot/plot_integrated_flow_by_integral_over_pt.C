...
TFile *outFile = new TFile("../CompareResult/test.root","recreate");
TString outDirName={"old_UrQMD_7.7GeV"};
...
void v2plot_integrated_flow(){
  ...
  for (int id=0;id<npid;id++){
    ...
    grIntFlowVsCent[3][id] = new TGraphErrors(ncent,bin_cent,&vV22Gapint[0],bin_centE,&eV22Gap[0]);
...
void plot_integrated_flow_by_integral_over_pt(){
  CalStatErrCent1040();
  if (bMergeCharged){
    pidNames.clear();
    pidFancyNames.clear();
