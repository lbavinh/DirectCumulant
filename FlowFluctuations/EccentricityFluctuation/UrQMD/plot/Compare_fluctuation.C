void Compare_fluctuation(){
  ...
  {
    inputEcc[e] = new TFile(Form("ecc_UrQMD_%sGeV.root",energy[e].Data()),"read");
  ...
  TPaveLabel* title = new TPaveLabel(0.1,0.915,0.9,0.95,"UrQMD, Au+Au");
  ...
  for (int ipad=0;ipad<nenergy;ipad++){
    ...
    if (ipad==0) {
      ...
      leg_pt->AddEntry(grRatioV2[ipad][1],"v_{2}{4}/v_{2}{2}","p");
      leg_pt->AddEntry(grEcc[ipad],"#epsilon_{2}{4}/#epsilon_{2}{2}","p");
      leg_pt->Draw();      
