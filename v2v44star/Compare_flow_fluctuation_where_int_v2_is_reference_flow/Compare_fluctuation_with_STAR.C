void Compare_fluctuation_with_STAR(){
  ...
  const float maxV2Ratio = 1.35;
  const float minV2Ratio = 0.6;
  const float leg_coordinate[4]={0.17,0.7,0.35,0.99};
  const float labelSize = 0.08;
  // const TString leg_header[2] = {"UrQMD,20-30%","SMASH,20-30%"};
  const TString leg_header[4] = {"4.5 GeV","7.7 GeV","11.5 GeV","19.6 GeV"};
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int imod=0;imod<nmodel;imod++){
      ...
      for (int imeth=0;imeth<nmethod;imeth++){ // v24 = 1; v22(eta-gap) = 3
        grV2[ien][imod][imeth] = (TGraphErrors*)input[ien][imod]->Get(Form("gr_cent2_%i_0",imeth)); // "_0" - Charged Hadrons
      }
    }
  }
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int imod=0;imod<nmodel;imod++){
      ...
      grRatioV2[ien][imod] -> SetTitle(Form("%s;p_{T} [GeV/c];v_{2}{4}/v_{2}{2}",model[imod].Data()));
      
    }
    // grRatioV2[ien][0] -> SetMarkerStyle(kFullCircle);
    // grRatioV2[ien][1] -> SetMarkerStyle(kFullCircle);

    grRatioV2[ien][0] -> SetMarkerColor(kGreen);
    grRatioV2[ien][0] -> SetLineColor(kGreen);
  ...
  TString energySTAR[3]={"77","115","196"};
  ...
  for (int iener=0;iener<nenergy;iener++){
    FILE *fp1;
    char path1[800];
    sprintf(path1,"./v2methods%sptCent20-30.txt",energySTAR[iener].Data());
    ...
    grRatioV2STAR[iener] = new TGraphErrors(npt,pt,ratiov24v22,pte,ratiov24v22e);
    grRatioV2STAR[iener] -> SetTitle(Form("STAR data;p_{T} [GeV/c];v_{2}{4}/v_{2}{2}"));
    grRatioV2STAR[iener] -> SetMarkerStyle(kFullCircle);
    grRatioV2STAR[iener] -> SetMarkerSize(1.5);
    grRatioV2STAR[iener] -> SetMarkerColor(kRed);
    grRatioV2STAR[iener] -> SetLineColor(kRed);
  ...
  //=================================================================================
  can->cd(1);
  TH2F *h1 = new TH2F("h1",";p_{T} [GeV/c];v_{2}{4}/v_{2}{2}",1,minpt,maxpt,1,minV2Ratio,maxV2Ratio);
  ...
  //=================================================================================
  can->cd(2);
  TH2F *h2 = new TH2F("h2",";p_{T} [GeV/c];v_{2}{4}/v_{2}{2}",1,minpt,maxpt,1,minV2Ratio,maxV2Ratio);
  ...
  //=================================================================================
  can->cd(3);
  TH2F *h3 = new TH2F("h3",";p_{T} [GeV/c];v_{2}{4}/v_{2}{2}",1,minpt,maxpt,1,minV2Ratio,maxV2Ratio);
  ...
  //=================================================================================
  can->cd(4);
  TH2F *h4 = new TH2F("h4",";p_{T} [GeV/c];v_{2}{4}/v_{2}{2}",1,minpt,maxpt,1,minV2Ratio,maxV2Ratio);
  ...
  leg_pt4->Draw();
  lineOne.DrawLine(minpt,1.,maxpt,1.);
  can->SaveAs("ratiov2pt_vsSTAR.png");
