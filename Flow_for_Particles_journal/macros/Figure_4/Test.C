...
TString input3 = Form("ThirdRun_AMPT15_%s.root",energy.Data());
void Test(TString inputFirstRunFileName = input1, TString inputSecondRunFileName = input2, TString inputThirdRunFileName = input3)
{
  ...
  bool bUseProduct = 1;
  Int_t nmethod = 8;
  TString title[]={"#it{v}_{2}{#Psi_{2,TPC}}","#it{v}_{2}^{SP}{Q_{2,TPC}}","#it{v}_{2}{2,#eta-gap}","#it{v}_{2}{2}","#it{v}_{2}{4}","#it{v}_{2}{LYZ, Sum}","#it{v}_{2}{LYZ}", "#it{v}_{2}{LYZ, EP}"};
  ...
  TFile *firun3 = new TFile(inputThirdRunFileName.Data(),"read");
  auto *prV2EP3D = (TProfile3D*) firun2->Get("prV2EtaSubEventPlane");
  auto *prV2SP3D = (TProfile3D*) firun2->Get("prV2ScalarProduct");
  auto *prV2LYZEP3D = (TProfile3D *)firun3->Get("prV2LYZEP");
  for (int i = 0; i< ncent; i++)
  {
    TProfile *prV2EPInt = PlotV2EPDifferentialVersusPt(prV2EP3D,bin_cent[i],bin_cent[i+1],eta_cut);
    TProfile *prV2SPInt = PlotV2SPDifferentialVersusPt(prV2SP3D,bin_cent[i],bin_cent[i+1],eta_cut);
    TProfile *prV2LYZInt = PlotV2LYZEPDifferentialVersusPt(prV2LYZEP3D,bin_cent[i],bin_cent[i+1],eta_cut);
    graph[i][0] = Converter(prV2EPInt);
    graph[i][1] = Converter(prV2SPInt);
    graph[i][7] = Converter(prV2LYZInt);  
  }
  ...
  TPaveLabel* label = new TPaveLabel(0.1,0.95,0.9,0.99,Form("AMPT, #it{#sigma}_{p}=1.5 mb, Au+Au at #sqrt{s_{NN}} = %s GeV, Charged Hadrons",energy.Data()));
  ...
  for (int ic=firstCentToDraw; ic<numberOfCentToDraw+firstCentToDraw; ic++)
  {
    ...
    {
    TLegend *leg1 = new TLegend(0.2,0.6,0.8,0.8);
  ...
  for (int ic=firstCentToDraw; ic<firstCentToDraw+numberOfCentToDraw; ic++)
  {
    ...
    can->cd(padID);
    vGr_ratio[ic].at(6)->Draw("AP PLC PMC");
    if (ic==firstCentToDraw)
    {
    vGr_ratio[ic].at(6)->GetYaxis()->SetTitleSize(titleSize);
    vGr_ratio[ic].at(6)->GetYaxis()->SetLabelSize(labelSize);
    vGr_ratio[ic].at(6)->GetYaxis()->SetTitleFont(textFont);
    vGr_ratio[ic].at(6)->GetYaxis()->SetLabelFont(textFont);
    vGr_ratio[ic].at(6)->GetXaxis()->SetTitleSize(titleSize);
    vGr_ratio[ic].at(6)->GetXaxis()->SetLabelSize(labelSize);
    vGr_ratio[ic].at(6)->GetXaxis()->SetTitleFont(textFont);
    vGr_ratio[ic].at(6)->GetXaxis()->SetLabelFont(textFont);
    vGr_ratio[ic].at(6)->GetYaxis()->SetTitleOffset(titleOffSet);
    vGr_ratio[ic].at(6)->GetXaxis()->SetTitleOffset(1.);
    vGr_ratio[ic].at(6)->GetXaxis()->SetLabelOffset(0.017);
    tex.SetTextSize(titleSize);
    }
    else
    {
    vGr_ratio[ic].at(6)->GetYaxis()->SetTitleSize(titleSize+0.015);
    vGr_ratio[ic].at(6)->GetYaxis()->SetLabelSize(labelSize+0.015);
    vGr_ratio[ic].at(6)->GetYaxis()->SetTitleFont(textFont);
    vGr_ratio[ic].at(6)->GetYaxis()->SetLabelFont(textFont);
    vGr_ratio[ic].at(6)->GetXaxis()->SetTitleSize(titleSize+0.015);
    vGr_ratio[ic].at(6)->GetXaxis()->SetLabelSize(labelSize+0.015);
    vGr_ratio[ic].at(6)->GetXaxis()->SetTitleFont(textFont);
    vGr_ratio[ic].at(6)->GetXaxis()->SetLabelFont(textFont);
    vGr_ratio[ic].at(6)->GetYaxis()->SetTitleOffset(titleOffSet-0.1);
    vGr_ratio[ic].at(6)->GetXaxis()->SetTitleOffset(0.8);
    tex.SetTextSize(titleSize+0.015);
    }
    ...
    vGr_ratio[ic].at(6)->SetTitle(Form(";#it{p}_{T}, GeV/c;#it{v}_{2}{LYZ}/%s", title[ratioToMethod].Data()));
    
    tex.DrawLatex(0.2,1.08,padName[padID-1].Data());
  }
  can->SaveAs(Form("Test_CompareMethods_Diff_v2{4}_vs_v2{LYZ,Prod}_at_%s.png",energy.Data()));
  can->SaveAs(Form("Test_CompareMethods_Diff_v2{4}_vs_v2{LYZ,Prod}_at_%s.pdf",energy.Data()));
