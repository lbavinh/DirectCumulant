void Comparev2PT(){ // 0: v22; 1:v24; 2: v2EP, 3: gapped v22
  ...
  const float minpt = -0.15;
  const float maxpt = 3.15;
  ...
  TString graphTitle[]={"v_{2}{4}","v_{2}{2}","v_{2}^{EP}{#Psi_{2,TPC}}","v_{2}^{SP}{Q_{2,TPC}}","v_{2}^{EP}{#Psi_{1,FHCal}}"};
  ...
  for (int ipad=0;ipad<10;ipad++){
    ...
    if (ipad<5)tex.DrawLatex(maxpt*0.7,maxV2*0.95,Form("%s",graphTitle[ipad].Data())); // 0.95-0.025
    ...
    for (int ilev=0;ilev<nlevel;ilev++){
      for (int id=0;id<npid;id++){
        ...
        if (ipad==7) grV2[id][ilev][1][1]->Draw("P"); // PZ
