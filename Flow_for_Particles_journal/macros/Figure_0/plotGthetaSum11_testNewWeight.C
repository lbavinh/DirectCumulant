...
void plotGthetaSum11_testNewWeight()
{
  TFile *fi = new TFile("../FirstRun_11.5_new.root","read");
  const int ncent = 9;
  const int rbins = 500;
  ...
  tex.DrawLatex(55,0.02,"#splitline{AMPT SM, #it{#sigma}_{p}=1.5 mb}{Au+Au at #sqrt{#it{s}_{NN}} = 11.5 GeV}");

  gPad->SetLogy();
  gStyle->SetOptStat(0);
  c.SaveAs("Gtheta115_testNewWeight.pdf");
  c.SaveAs("Gtheta115_testNewWeight.png");
}
