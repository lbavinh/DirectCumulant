...
#include "constants.C"
void CompareFHCal_versus_MCEP()
{

  Bool_t saveAsPNG = true;
  TString inputFileName[1];
  // inputFileName[0] = "../data/SecondRun_UrQMD_7.7_Reco_motherID_16hits.root";
  // inputFileName[0] = "../data/SecondRun_UrQMD_7.7_Reco_2sigma_16_TPChits.root";
  inputFileName[0] = "../data/SecondRun_UrQMD_7.7_Model.root";


  TString outDirName = "CompareFHCal_versus_MCEP";
  ...
  TString methodName[] = {"FHCalEP", "MC"}; // FHCalEP
  TString title[]={"v_{2}{#Psi_{1,FHCal}}","v_{2}{MC-EP}"};
  const Int_t markerStyle[] = {20,24,22,22,24};
  ...
  for (Int_t id=0;id<npid;id++)
  {
    ...
    {
      auto prV2CentPt = (TProfile2D *)fi[0]->FindObjectAny(Form("prV2%svsPt_pid%i",methodName[i].Data(),id));
    ...
    TCanvas *can = (TCanvas*)DrawTGraph(vGr,"10-40%",0.89, 1.11, minpt, maxpt, -0.005, 0.13,
                                        0.2, 0.4, 0.4, 0.88,
