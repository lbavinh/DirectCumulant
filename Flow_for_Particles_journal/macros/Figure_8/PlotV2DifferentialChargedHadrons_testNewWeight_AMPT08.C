...
void PlotV2DifferentialChargedHadrons_testNewWeight_AMPT08()
{
  ...
  bool bUseProduct = 1;
  Int_t nmethod = 7;
  TString title[]={"#it{v}_{2}{#Psi_{2,TPC}}","#it{v}_{2}^{SP}{Q_{2,TPC}}","#it{v}_{2}{2,#eta-gap}","#it{v}_{2}{2}","#it{v}_{2}{4}","#it{v}_{2}{LYZ, Sum}","#it{v}_{2}{LYZ}"};
  // 0-EP, 1-SP, 2-2eta-gap, 3-2, 4-4, 5-LYZ(Sum), 6-LYZ(Prod)
  const int nenergy = 2;
  TGraphErrors *graph[ncent][nmethod][nenergy];

  const TString energy[nenergy] = {"7.7","11.5"};
  for (int ien = 0; ien < nenergy; ien++)
  {
    TString input1 = Form("../FirstRun_AMPT08_%s_new.root",energy[ien].Data());
    TString input2  = Form("../SecondRun_AMPT08_%s_new.root",energy[ien].Data());

    TString inputFirstRunFileName = input1;
    TString inputSecondRunFileName = input2;
    ...
    for (int i = 0; i< ncent; i++)
    {
      ...
      graph[i][0][ien] = Converter(prV2EPInt);
      graph[i][1][ien] = Converter(prV2SPInt);  
    }
    ...
    for (int icent=0; icent<ncent; icent++){ // loop over centrality classes
      ...
      graph[icent][2][ien] = new TGraphErrors(npt, X, v2Dif[0][icent][0], errX, v2eDif[0][icent][0]); // v2{2}
      graph[icent][3][ien] = new TGraphErrors(npt, X, v2Dif[1][icent][0], errX, v2eDif[1][icent][0]); // v2{4}
      graph[icent][4][ien] = new TGraphErrors(npt, X, v2Dif[2][icent][0], errX, v2eDif[2][icent][0]); // v2{2,eta-gap}
    }
    ...
    for (int ic = 0; ic < ncent; ic++)
    {
      graph[ic][5][ien] = new TGraphErrors(npt, X, v2diff[ic], errX, v2diffe[ic]);
      graph[ic][6][ien] = new TGraphErrors(npt, X, v2diffPro[ic], errX, v2diffePro[ic]);
    }
    for (int ic = 0; ic < ncent; ic++)
    {
      for (int i=0; i<nmethod; i++)
      {
      graph[ic][i][ien]->RemovePoint(0);
      graph[ic][i][ien]->SetTitle(title[i].Data());
      graph[ic][i][ien]->SetMarkerStyle(markerStyle[i]);
      graph[ic][i][ien]->SetMarkerSize(markerSize);
      graph[ic][i][ien]->GetXaxis()->SetTitle("#it{p}_{T}, GeV/c");
      graph[ic][i][ien]->GetYaxis()->SetTitle("#it{v}_{2}");
      graph[ic][i][ien]->SetDrawOption("P PLC PMC");
      graph[ic][i][ien]->GetXaxis()->SetLimits(minX,maxX);
      graph[ic][i][ien]->GetYaxis()->SetRangeUser(minY,maxY);
      // graph[ic][i][ien]->GetYaxis()->SetNdivisions(504);
      // graph[ic][i][ien]->GetXaxis()->SetNdivisions(504);
      }
    }
  }
  ...
  for (int ic = 0; ic < ncent; ic++)
  {
    for (int i=0; i<nmethod; i++)
    {
      vGr[ic].push_back(graph[ic][i][0]);
    }
  }
  vector<TGraphErrors*> vGr2[ncent];
  for (int ic=0; ic<ncent; ic++)
  {
    for (int i=0; i<nmethod; i++)
    {
      vGr2[ic].push_back(graph[ic][i][1]);
    }
  }

  TCanvas *can = new TCanvas("can","",1200,700);
  TPaveLabel* label = new TPaveLabel(0.1,0.95,0.9,0.99,Form("AMPT SM, #it{#sigma}_{p}=0.8 mb, Au+Au, Charged Hadrons"));//at 
  ...
  for (int ic=firstCentToDraw; ic<numberOfCentToDraw+firstCentToDraw; ic++)
  {
    ...
    if (ic==firstCentToDraw) tex.DrawLatex(0.2,0.18,"#sqrt{#it{s}_{NN}} = 7.7 GeV");
    tex.DrawLatex(0.55,0.22,Form("%i-%i%%",centrality.at(ic).first, centrality.at(ic).second));
    if (ic==firstCentToDraw+1)
    {
    TLegend *leg1 = new TLegend(0.1,0.6,0.6,0.8);
    ...
    // {
    //   TLegend *leg2 = new TLegend(0.25,0.1,0.85,0.2);
    //   leg2->SetBorderSize(0);
    //   leg2->SetTextSize(titleSize+0.015);
    //   for (int i=5; i<vGr[ic].size(); i++)
    //   {
    //     if (i==excludeMethod1 || i==excludeMethod2 || i==excludeMethod3 || i==excludeMethod4 || i==excludeMethod5 || i==excludeMethod6) continue;
  ...
  for (int ic=firstCentToDraw; ic<firstCentToDraw+numberOfCentToDraw; ic++)
  {
    ...
    can->cd(padID);
    vGr2[ic].at(ratioToMethod)->Draw("AP PLC PMC");
    if (ic==firstCentToDraw)
    {
      vGr2[ic].at(ratioToMethod)->GetYaxis()->SetTitleSize(titleSize);
      vGr2[ic].at(ratioToMethod)->GetYaxis()->SetLabelSize(labelSize);
      vGr2[ic].at(ratioToMethod)->GetYaxis()->SetTitleFont(textFont);
      vGr2[ic].at(ratioToMethod)->GetYaxis()->SetLabelFont(textFont);
      vGr2[ic].at(ratioToMethod)->GetXaxis()->SetTitleSize(titleSize);
      vGr2[ic].at(ratioToMethod)->GetXaxis()->SetLabelSize(labelSize);
      vGr2[ic].at(ratioToMethod)->GetXaxis()->SetTitleFont(textFont);
      vGr2[ic].at(ratioToMethod)->GetXaxis()->SetLabelFont(textFont);
      vGr2[ic].at(ratioToMethod)->GetYaxis()->SetTitleOffset(titleOffSet);
      vGr2[ic].at(ratioToMethod)->GetXaxis()->SetTitleOffset(1.);
      vGr2[ic].at(ratioToMethod)->GetXaxis()->SetLabelOffset(0.017);
      tex.SetTextSize(titleSize);
    }
    else
    {
      vGr2[ic].at(ratioToMethod)->GetYaxis()->SetTitleSize(titleSize+0.01);
      vGr2[ic].at(ratioToMethod)->GetYaxis()->SetLabelSize(labelSize+0.01);
      vGr2[ic].at(ratioToMethod)->GetYaxis()->SetTitleFont(textFont);
      vGr2[ic].at(ratioToMethod)->GetYaxis()->SetLabelFont(textFont);
      vGr2[ic].at(ratioToMethod)->GetXaxis()->SetTitleSize(titleSize+0.01);
      vGr2[ic].at(ratioToMethod)->GetXaxis()->SetLabelSize(labelSize+0.01);
      vGr2[ic].at(ratioToMethod)->GetXaxis()->SetTitleFont(textFont);
      vGr2[ic].at(ratioToMethod)->GetXaxis()->SetLabelFont(textFont);
      vGr2[ic].at(ratioToMethod)->GetYaxis()->SetTitleOffset(titleOffSet-0.1);
      vGr2[ic].at(ratioToMethod)->GetXaxis()->SetTitleOffset(0.8);
      tex.SetTextSize(titleSize+0.01);
    }
    

    lineOne.DrawLine(minX,1.,maxX,1.);
    for (int i=0; i<vGr2[ic].size(); i++)
    {
      if (i==excludeMethod1 || i==excludeMethod2 || i==excludeMethod3 || i==excludeMethod4 || i==excludeMethod5 || i==excludeMethod6 || i==ratioToMethod) continue;
      vGr2[ic].at(i)->Draw("P PLC PMC");
    }
    vGr2[ic].at(ratioToMethod)->SetTitle(Form(";#it{p}_{T}, GeV/c;#it{v}_{2}"));
    
    tex.DrawLatex(0.2,0.22,padName[padID-1].Data());
    if (ic==firstCentToDraw) tex.DrawLatex(0.2,0.18,"#sqrt{#it{s}_{NN}} = 11.5 GeV");
  }
  can->SaveAs(Form("V2pT_LYZ_AMPT_testNewWeight_AMPT08.png"));
  can->SaveAs(Form("V2pT_LYZ_AMPT_testNewWeight_AMPT08.pdf"));
