...
void PlotV2(TString inputFileName1 = "FirstRun.root", TString inputFileName2 = "SecondRun.root")
{
      ...
      if (dVtheta[ic][thetabin] != 0)
      {
        ...
        for (int ipt = 0; ipt < npt; ipt++)
        {
          ...
          if (cDenominator.Rho()!=0) {
            ...
            if (bDebug) cout << "At cent: " << ic <<" , theta: " << thetabin <<" cDenominator=0" << endl;
          }
        }
      }
    }
  ...
  hv2EP->GetXaxis()->SetLimits(0,60.);
  hLYZ->GetXaxis()->SetLimits(0,60.);
  hV22->GetXaxis()->SetLimits(0,60.);
  ...
  hv2EPpt[centrality]->GetYaxis()->SetRangeUser(-0.04,0.26);
  ...
  tex.DrawLatex(0.5,0.24,legHeader[centrality].Data());
  TLegend *leg2 = new TLegend(0.1,0.5,0.4,0.85);
  leg2->SetBorderSize(0);
