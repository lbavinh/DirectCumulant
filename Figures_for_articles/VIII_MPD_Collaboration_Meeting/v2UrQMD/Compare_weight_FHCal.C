...
void Compare_weight_FHCal()
{
  ...
  bool bUseProduct = 1;
  Int_t nmethod = 2;
  TString title[]={"E weight","p_{T} weight"};
  ...
  for (int ic = 0; ic < 1; ic++)
  {
    ...
    TCanvas *can = (TCanvas*)DrawTGraph(vGr[ic],Form("%i-%i%%",centlow,centhigh),0.89, 1.11, minpt, 3.6, -0.005, 0.25,
                                      // 0.65, 0.05, 0.9, 0.5,
                                      0.2, 0.65, 0.4, 0.88,
                                      "UrQMD, Au+Au", Form("#sqrt{s_{NN}} = 7.7 GeV, h^{#pm}, |#eta|<%1.1f",eta_cut),1,"Ratio to E-weight");
  can->SetName(Form("10-40%%"));
  can->SaveAs(Form("CompareWeight_v2_FHCALEP_%i%i.pdf",centlow,centhigh));
  }
