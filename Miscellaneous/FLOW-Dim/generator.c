...
for(int i=0;i<run0;i++)
    {
    if (i%1000==0){cout << i<< " ivents were simulated,the process is "<<i*100/run0<<"% complete" <<endl;}
    Nch=20;//(int)nLambda->Poisson(20);
    psi_RP=psiRP_dist->GetRandom();
    //Moment=momentdist->GetRandom();
    //phi_dist->FixParameter(2,V2(Moment));
    phi_dist->FixParameter(3,psi_RP);
    for(int j=0;j<Nch;j++)
        {
        moment[j]=momentdist->GetRandom();
        phi_dist->SetParameter(2,V2(moment[j]));
        //phi_dist->SetParameter(2, V2(moment[j]));
