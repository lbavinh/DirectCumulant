  ...
  if (fUseProduct)
  {
    ...
    else
    {
      for (int it = 0; it < thetabins; ++it)
      {
        ...
        fGenfunPror0[it] *= TComplex(1.0, pt * fR02Pro[icent][it] * dCosTerm);
        // TComplex cCosTermComplex(1., fR02Pro[icent][it] * dCosTerm);
        TComplex cCosTermComplex(1., pt * fR02Pro[icent][it] * dCosTerm);
        // fdGr0[it] += (dCosTerm / cCosTermComplex);
        fdGr0[it] += (pt * dCosTerm / cCosTermComplex);
      }
    }
  }
}
