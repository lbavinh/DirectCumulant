...
double pt_min=0.,pt_max=3.,eta_max=2.5;
...
psiRP=phi2;
 for (int i=0;i<nh;i++)
    ...
    if(bimp>=b_bin[7] && bimp<=b_bin[8]){
        if(pt>=pt_min && pt<=pt_max && eta<-0.05 && eta>-eta_max){
        Q+=TComplex(cos(n*(phi)),sin(n*(phi)));
        Q2+=TComplex(cos(2*n*(phi)),sin(2*n*(phi)));
        MCQ+=cos(n*(phi-psiRP));M+=1;}
    
        for(int m=0;m<NN;m++){
        if(pt>=pt_bin[m] && pt<pt_bin[m+1] && eta>0.05 && eta<eta_max){
        mp[m]+=1;
        p2[m]+=TComplex(cos(2*n*(phi)),sin(2*n*(phi)));
        p[m]+=TComplex(cos(n*(phi)),sin(n*(phi)));
        QMC[m]+=(cos(n*(phi-psiRP)));}
        
        if((pt>=pt_bin[m] && pt<pt_bin[m+1] && eta>0.05 && eta<eta_max) && (pt>=pt_min && pt<=pt_max && eta<-0.05 && eta>-eta_max)){
        mq[m]+=1;
...
if(DiffMQ2[s]==0 || DiffMQ4[s]==0 || MQ2==0 || MQ4==0) break;
