...
void ChainInit(){
  char list[500], chain[500];
  sprintf(list,"/weekly/povarov/lbavinh/PicoDst/runlist_PicoDst.list");
  for (int i=0; i<386; i++){
    sprintf(chain,"/weekly/povarov/lbavinh/PicoDst/chain/chain%i.root",i);
    MakeChain(list,chain,(i*13)+1,(i+1)*13);
  }
  cout << "Tree chains were created successfully!" << endl;
}