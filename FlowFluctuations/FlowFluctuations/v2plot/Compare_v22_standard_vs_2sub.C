...
const double maxpt = 2.6; // for v2 vs pt plotting
...
vector <Double_t> coordinateLeg = {0.18,0.5,0.45,0.789};
vector<pair<Double_t,Double_t>> rangeRatio = {{0.81,1.16},{0.81,1.16}};
vector<pair<Double_t,Double_t>> rangeRatioRF ={{0.84,1.21},{0.84,1.21}};
int marker[nmethod]={21,20,25,22}; // 2QC, 4QC, EP, 2QC-gapped
...
void Compare_v22_standard_vs_2sub(TString model = "SMASH", TString energy = "11.5GeV"){
  ...
  TString level= (TString) Form("%s, Au+Au at #sqrt{s_{NN}}=%s","SMASH",energy.Data()); // model.Data() AMPT SM, #sigma_{p}=1.5mb
  ...
  const char *grTitleDF[nmethod]={"v_{2}{2};p_{T} [GeV/c];v_{2}",
                                  ...
                                  "v_{2}{2,|#Delta#eta|>0.1};p_{T} [GeV/c];v_{2}"};
  outFile -> cd();
  for (int imeth=0; imeth<nmethod; imeth++){
    for (int id=0;id<npid;id++){
  ...
  for (int id=0;id<npid;id++){
    ...
    vgrv2pt1040.push_back(grDifFl1040[0][id]);
  ...
  const char *grTitleRF[nmethod]={"v_{2}{2};centrality (%);v_{2}",
                                  ...
                                  "v_{2}{2,|#Delta#eta|>0.1};centrality (%);v_{2}"};
  outFile -> cd();
  for (int imeth=0; imeth<nmethod; imeth++){
  ...
  vgrv2cent_chargedHardons.push_back(grRefFl[0]);
  ...
  cV2CentRF = (TCanvas*) DrawTGraph(vgrv2cent_chargedHardons,"",rangeRatioRF.at(0).first, rangeRatioRF.at(0).second, mincent, maxcent, minV2int, maxV2int,
                                    coordinateLeg.at(0), coordinateLeg.at(1), coordinateLeg.at(2), coordinateLeg.at(3),
                                    level.Data(), Form("Ch. hadrons, %1.1f<p_{T}<%1.1f GeV/c, |#eta|<1.5",minptRFP,maxptRFP),
                                    true, "v_{2}{2}");
