...
void Sigma_vsCent_11GeV_vHLLEUrQMD()
{
  const int ncent = 9;
  ...
  TFile *fiFlow = new TFile("vHLLEUrQMD_11.5GeV.root","read");
  ...
  l->AddEntry(grSigmaV2[1],Form("%s\\ (vHLLE+UrQMD)",grSigmaV2[1]->GetTitle()),"p");
  ...
  c.SaveAs("Sigma_vsCent_11GeV_vHLLEUrQMD.png");
