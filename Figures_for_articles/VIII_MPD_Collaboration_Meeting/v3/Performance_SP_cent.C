...
#include "constants.C"
// vector<TGraphErrors*>*
void Performance_SP_cent(TString inputMC = "SecondRun_vHLLEUrQMD_11.5_MC.root", TString inputRECO = "SecondRun_vHLLEUrQMD_11.5_Reco.root")
{
  Double_t ptlow = 0.2;
  Double_t pthigh = 3.0;
  ...
  for (int i = 0; i < 1; i++)
  {
    TProfile *prV2SP_MC = PlotV2ScalarProductIntegrated(prV2SP3D_MC,ptlow,pthigh,eta_cut);
    TProfile *prV2SP_RECO = PlotV2ScalarProductIntegrated(prV2SP3D_RECO,ptlow,pthigh,eta_cut);
  ...
  for (int ic = 0; ic < 1; ic++)
  {
    ...
    graph[ic][i]->GetXaxis()->SetTitle("Centrality, %");
    graph[ic][i]->GetYaxis()->SetTitle("v_{3}{SP}");
  ...
  for (int ic = 0; ic < 1; ic++)
  {
    ...
    TCanvas *can = (TCanvas*)DrawTGraph(vGr[ic],"",0.89, 1.11, 0., 80., -0.005, 0.02,
                                      // 0.65, 0.05, 0.9, 0.5,
                                      0.2, 0.65, 0.4, 0.88,
                                      "vHLLE+UrQMD, Au+Au", Form("#sqrt{s_{NN}} = 11.5 GeV, h^{#pm}, |#eta|<%1.1f",eta_cut),1,"Reco/MC");
  can->SetName(Form("10-40%%"));
  can->SaveAs(Form("Performance_v3_SP.pdf"));
  }
