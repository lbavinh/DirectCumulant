void ThreeEnergy(){
  const int nenergy = 2; // 4.5, 7.7, 11.5
  ...
  const float leg_coordinate[4]={0.08,0.22,0.45,0.36}; //  0.05,0.7,0.35,0.99
  ...
  for (int ien=0;ien<nenergy;ien++){
    ...
    grRatioV2[ien][5] -> SetMarkerColor(kBlue+3);
    grRatioV2[ien][5] -> SetLineColor(kBlue+3);
    ...
    grRatioV2[ien][1] -> SetMarkerStyle(kOpenTriangleUp); // kFullTriangleUp

    grRatioV2[ien][2] -> SetMarkerColor(kBlack);
    grRatioV2[ien][2] -> SetLineColor(kBlack);
    grRatioV2[ien][2] -> SetMarkerStyle(kOpenCross);

    grRatioV2[ien][3] -> SetMarkerColor(kYellow+3);
    grRatioV2[ien][3] -> SetLineColor(kYellow+3);
    grRatioV2[ien][3] -> SetMarkerStyle(kOpenCrossX); // kFullCrossX
  ...
  for (int iener=0;iener<nenergy;iener++){
    ...
    grRatioV2[iener][0] -> SetMarkerSize(markerSize+1.5);
  ...
  // ========================================================================
  TFile *fi45[2]; // UrQMD, SMASH
  TGraphErrors *gr45[2][nmethod], *grRatioV2_45[2];
  for (int imod=0; imod<2; imod++){
    fi45[imod] = new TFile(Form("./Data/v2_%s_4.5GeV.root",model[imod+1].Data()),"read");
    for (int imeth=0;imeth<nmethod;imeth++){ // v22, v24, v2(eta-sub), v22(eta-gap)
      gr45[imod][imeth] = (TGraphErrors*)fi45[imod]->Get(Form("grRF_%i_0",imeth)); // "_0" - Charged Hadrons
      gr45[imod][imeth] -> RemovePoint(0);
    }  
  }
  Double_t *vx_gr_45[2][nmethod], *vy_gr_45[2][nmethod], *ex_gr_45[2][nmethod], *ey_gr_45[2][nmethod];
  Int_t nbins_45[2][nmethod];

  for (int imod=0;imod<2;imod++){
    for (int imeth=0;imeth<nmethod;imeth++){
      // Read points
      vx_gr_45[imod][imeth]=(Double_t*)gr45[imod][imeth]->GetX();
      vy_gr_45[imod][imeth]=(Double_t*)gr45[imod][imeth]->GetY();

      // Read errors
      ex_gr_45[imod][imeth]=(Double_t*)gr45[imod][imeth]->GetEX();
      ey_gr_45[imod][imeth]=(Double_t*)gr45[imod][imeth]->GetEY();

      nbins_45[imod][imeth]=(Int_t) gr45[imod][imeth]->GetN();
    }
  }
  ...
  for (int imod=0;imod<2;imod++){
    ...
    for (int i=0;i<nbins_45[imod][3];i++){
      Double_t ratio = vy_gr_45[imod][1][i]/vy_gr_45[imod][3][i];
      Double_t ratioErr = ratio*(TMath::Sqrt(TMath::Power(ey_gr_45[imod][3][i]/vy_gr_45[imod][3][i],2)+TMath::Power(ey_gr_45[imod][1][i]/vy_gr_45[imod][1][i],2)));
      vRatio.push_back(ratio);
      vRatioErr.push_back(ratioErr);
    }
    grRatioV2_45[imod] = new TGraphErrors(nbins_45[imod][3],vx_gr_45[imod][3],&vRatio[0],ex_gr_45[imod][3],&vRatioErr[0]);      
  }

  grRatioV2_45[0] -> SetMarkerColor(kGreen+3);
  grRatioV2_45[0] -> SetLineColor(kGreen+3);
  grRatioV2_45[0] -> SetMarkerStyle(kOpenTriangleUp); // kFullTriangleUp

  grRatioV2_45[1] -> SetMarkerColor(kBlack);
  grRatioV2_45[1] -> SetLineColor(kBlack);
  grRatioV2_45[1] -> SetMarkerStyle(kOpenCross);

  for (int imod=0;imod<2;imod++){

    grRatioV2_45[imod]->SetMarkerSize(markerSize);
  ...
  gStyle->SetOptStat(0);
  can->Divide(3,1,0,0);
  TString energy3[3]={"4.5","7.7","11.5"};
  TH2F *h[3];
  for (int ipad=0;ipad<3;ipad++){
    ...
    if (ipad==0) {
      ...
      leg_pt->AddEntry(grRatioV2[0][1],legendEntries[1].Data(),"p"); // UrQMD
      leg_pt->AddEntry(grRatioV2[0][2],legendEntries[2].Data(),"p"); // SMASH
      
      leg_pt->Draw();
      tex.SetTextSize(labelSize1-0.02);
      tex.SetTextAlign(11);
      // tex.DrawLatex(mincent+13,minV2Ratio+0.038,"Phys. Rev. C 86, 054908 (2012)");
      ...
      leg_pt->AddEntry(grRatioV2[0][5],legendEntries[5].Data(),"p"); // vHLLE+UrQMD
      leg_pt->AddEntry(grRatioV2[0][0],legendEntries[0].Data(),"p"); // STAR
      leg_pt->Draw();
    }
    else if (ipad==2){
      ...
      leg_pt->SetTextFont(textFont);
      leg_pt->SetTextSize(labelSize-0.02);
      leg_pt->AddEntry(grRatioV2[0][3],legendEntries[3].Data(),"p"); // AMPT15
      leg_pt->AddEntry(grRatioV2[0][4],legendEntries[4].Data(),"p"); // AMPT08
      leg_pt->Draw();
    ...
    if (ipad!=0){
      for (int imod=0;imod<nmodel;imod++){
        grRatioV2[ipad-1][imod]->Draw("P");
      }
    }else{
      for (int imod=0;imod<2;imod++){
        grRatioV2_45[imod]->Draw("P");
      }
    }
  }
  can->SaveAs("Figure_Ratiov24v22_v2cent_4.5-11.5_compareWithSTAR.pdf");
  gROOT->SetStyle("Pub");
  can->SaveAs("Figure_Ratiov24v22_v2cent_4.5-11.5_compareWithSTAR.png");
