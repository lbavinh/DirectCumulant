...
Double_t CalCor22(TComplex Q2, Double_t M){ // single-event average 2-particle azimuthal correlation <2>
  
  Double_t nComb2  = M * (M-1);
  
  Double_t Q2Square = Q2.Rho2(); // Double_t Rho2() const {return fRe*fRe+fIm*fIm;}
...
Double_t CalCor24(TComplex Q2, TComplex Q4, Double_t M){ // single-event average 4-particle azimuthal correlation <4>

  Double_t nComb4  = M * (M-1) * (M-2) * (M-3);

  TComplex Q2Star   = TComplex::Conjugate(Q2); // static TComplex Conjugate(const TComplex &c) {return TComplex(c.Re(),-c.Im());}
...
inline bool exists(const std::string& name) {
  struct stat buffer; // struct stat is not defined within this code !!!
  return (stat (name.c_str(), &buffer) == 0); // c_str() return a const char* that points to a null-terminated string (i.e. a C-style string) (in C string is defined as: char MyString[20];)
}
...
int main (){
  ...
  Int_t           jeve;
  Float_t         bimp;
  Int_t           eid[6]; // this leave isn't used in calculations
  UInt_t          nTrk; // number of tracks
  Int_t           trkID[MAX]; // track ID
  Float_t         trkPx[MAX];
  Float_t         trkPy[MAX];
  Float_t         trkPz[MAX];
  Int_t           npart;
  ...
  TProfile *rpcor[NHAR][8]; // reaction plane particle correlation
  TProfile *rpcor2[NHAR][8];
  TProfile *rpcor3[NHAR][8];

  for(int ihar=0; ihar<2; ihar++){ // 16x3 TProfile
    for(int idet=0; idet<8; idet++){
      name<<"rpcor_"<<ihar+2<<"_"<<idet; // operator<< : Insert formatted output
  ...
  TH1D *hptdis = new TH1D("hptdis", "hptdis", 40, 0, 4.0); // pt (GeV/c)
  TH1D *hphidis = new TH1D("hphidis", "hphidis", 100, 0, PI2); // PI2 = 2*PI // phi
  TH1D *hetadis = new TH1D("hetadis", "hetadis", 120, -6.0, 6.0); // eta - pseudo-rapidity

  TH1D *htpcdis = new TH1D("htpcdis", "htpcdis", 200,  0, 200); // TPC distribution
  TH1D *hbbcdis = new TH1D("hbbcdis", "hbbcdis", 200,  0, 200); // BBC
  ...
  for(int irun=run*nrun; irun<(run+1)*nrun; irun++){ // nrun=10, run will be entered
  ...
  */
  ...
  for (int iev=0; iev<Nevt;iev++) { // event loop
    ch->GetEntry(iev);
    ...
    memset(re,0,sizeof(re)); // function of <cstring> : void * memset ( void * ptr, int value, size_t num ); fill 2 dimention vector re and rang with 0
