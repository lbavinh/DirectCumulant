void CompareV2Cent(){
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int imod=0;imod<nmodel;imod++){
      ...
      for (int imeth=0;imeth<nmethod;imeth++){ // v24 = 1; v22(eta-gap) = 3
        grV2[ien][imod][imeth] = (TGraphErrors*)input[ien][imod]->Get(Form("grRF_%i_%i",imeth,pid)); // "_0" - Charged Hadrons
      }
    }
  }
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int imod=0;imod<nmodel;imod++){
      ...
      grRatioV2[ien][imod] -> SetTitle(Form("%s;centrality [%%];v_{2}{4}/%s",model[imod].Data(),grFancyTitle[method].Data()));
  ...
  can->Divide(3,1,0,0);
  can->cd(1);
  TH2F *h1 = new TH2F("h1",Form(";centrality [%%];v_{2}{4}/%s",grFancyTitle[method].Data()),1,minpt,maxpt,1,minV2Ratio,maxV2Ratio);
  ...
  //=================================================================================
  can->cd(2);
  TH2F *h2 = new TH2F("h2",Form(";centrality [%%];v_{2}{4}/%s",grFancyTitle[method].Data()),1,minpt,maxpt,1,minV2Ratio,maxV2Ratio);
  ...
  //=================================================================================
  can->cd(3);
  TH2F *h3 = new TH2F("h3",Form(";centrality [%%];v_{2}{4}/%s",grFancyTitle[method].Data()),1,minpt,maxpt,1,minV2Ratio,maxV2Ratio);
  ...
  lineOne.DrawLine(minpt,1.,maxpt,1.);
  can->SaveAs(Form("Ratio_v24_%s_%s_v2cent.eps",grTitle[method].Data(),pidFancyNames[pid].Data()));
