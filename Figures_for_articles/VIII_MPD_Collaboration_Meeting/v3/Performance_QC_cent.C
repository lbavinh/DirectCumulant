...
#include "constants.C"
// vector<TGraphErrors*>*
void Performance_QC_cent(TString inputMC = "FirstRun_vHLLEUrQMD_11.5_MC.root", TString inputRECO = "FirstRun_vHLLEUrQMD_11.5_Reco.root")
{
  ...
  for (int ic = 0; ic < 1; ic++)
  {
    ...
    graph[ic][i]->GetYaxis()->SetTitle("v_{3}{2,|#Delta#eta|>0.1}");// 
  ...
  TCanvas *can = (TCanvas*)DrawTGraph(vGr[ic],"",0.79, 1.11, minpt, 60, -0.005, 0.1,
                                      // 0.65, 0.05, 0.9, 0.5,
                                      0.2, 0.65, 0.4, 0.88,
                                      "vHLLE+UrQMD, Au+Au", Form("#sqrt{s_{NN}} = 11.5 GeV, h^{#pm}, |#eta|<%1.1f",eta_cut),1,"Reco/MC");
  can->SetName(Form("10-40%%"));
  can->SaveAs(Form("Performance_v3_2QC.pdf"));
  }
