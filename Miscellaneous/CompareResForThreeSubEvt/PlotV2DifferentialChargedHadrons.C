...
void PlotV2DifferentialChargedHadrons()
{
  ...
  const double maxRatio = 1.12;
  const double minRatio = 0.88;
  ...
  bool bUseProduct = 1;
  Int_t nmethod = 2;
  TString title[]={"#it{v}_{2}{#Psi_{2,FHCal full}}","#it{v}_{2}{#Psi_{2,FHCal half}}"}; //  |#Delta#eta|>0.5    |#Delta#eta|>2
  TGraphErrors *graph[ncent][nmethod];
  TProfile *prV1_FHCalEP[ncent];
  TProfile *prV1_FHCalHalfEP[ncent];
  TFile *firun1 = new TFile("v2_FHCalEP.root","read");
  ...
  for (int ic=firstCentToDraw; ic<numberOfCentToDraw+firstCentToDraw; ic++)
  {
    ...
    {
    TLegend *leg1 = new TLegend(0.2,0.55,0.7,0.72);
  ...
  can->SaveAs(Form("v2pt_FHCalEPFull_vs_FHCalEPHalf.pdf"));
  gROOT->SetStyle("Pub");
  can->SaveAs(Form("v2pt_FHCalEPFull_vs_FHCalEPHalf.png"));
