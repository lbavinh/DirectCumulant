...
void Performance(){ // 0: v22; 1:v24; 2: v2EP, 3: gapped v22
  ...
  for (int ien=0;ien<nenergy;ien++){
    fi[0][ien] = new TFile(Form("v2_UrQMD_%s_Model.root",energyName[ien].Data()),"READ");
    fi[1][ien] = new TFile(Form("v2_UrQMD_%s_Reco.root",energyName[ien].Data()),"READ");
  ...
  for (int ien=0;ien<nenergy;ien++){
    fi[0][ien] = new TFile(Form("v2_UrQMD_%s_Model_EP.root",energyName[ien].Data()),"READ");
    fi[1][ien] = new TFile(Form("v2_UrQMD_%s_Reco_EP.root",energyName[ien].Data()),"READ");
  ...
  for (int imeth=0;imeth<nmethod;imeth++){
    for (int ien=0;ien<nenergy;ien++){
      ...
      grV2[2][0][imeth][ien]->SetMarkerStyle(kFullTriangleDown); // proton
      grV2[2][1][imeth][ien]->SetMarkerStyle(kOpenTriangleDown); // proton - reco
  ...
  TPaveLabel* title = new TPaveLabel(0.1,0.95,0.9,0.98,"UrQMD, Au+Au, 10-40%, reconstructed (GEANT4)");
  ...
  //   else if (ipad==2) h[ipad] = new TH2F(Form("pad_%i",ipad+1),Form(";%s;#it{v}_{2}",xAxisName.Data()),1,minpt,maxpt,1,minV2,maxV2);
  ...
  //   if (ipad<=4) tex.SetTextSize(labelSizeUpperPlot-0.03);
  //   else tex.SetTextSize(labelSizeLowerPlot-0.02);
  ...
  //     TLegend *leg_pt = new TLegend(leg_coordinate[0]-0.1,leg_coordinate[1]+0.2,leg_coordinate[2],leg_coordinate[3]+0.1);
  //     leg_pt->SetBorderSize(0);
  //     leg_pt->SetTextFont(textFont);
  //     leg_pt->SetTextSize(labelSizeUpperPlot-0.04);
  //     for (int ilev=0; ilev<nlevel; ilev++){
  ...
  //     TLegend *leg_pt1 = new TLegend(leg_coordinate[0]+0.15,leg_coordinate[1]+0.2,leg_coordinate[2],leg_coordinate[3]+0.1);
  //     leg_pt1->SetBorderSize(0);
  //     leg_pt1->SetTextFont(textFont);
  //     leg_pt1->SetTextSize(labelSizeUpperPlot-0.04);
  //     for (int id=0; id<npid; id++){
  ...
  //       if (ipad==1) grV2[id][ilev][1][0]->Draw("P"); // PZ
  //       if (ipad==2) grV2[id][ilev][2][0]->Draw("P");
  //       if (ipad==3) grV2[id][ilev][3][0]->Draw("P"); // PLC PMC // PLC (Palette Line Color) and PMC (Palette Marker Color)
  //       if (ipad==4) grV2[id][ilev][4][0]->Draw("P"); // PZ
  ...
  for (int ipad=0;ipad<10;ipad++){
    ...
    for (int ilev=0;ilev<nlevel;ilev++){
      for (int id=0;id<npid-1;id++){
        ...
        if (ipad==6) grV2[id][ilev][1][1]->Draw("P"); // PLC PMC // PLC (Palette Line Color) and PMC (Palette Marker Color)
        if (ipad==7) grV2[id][ilev][2][1]->Draw("P"); // PZ
        if (ipad==8) grV2[id][ilev][3][1]->Draw("P");
        if (ipad==9) grV2[id][ilev][4][1]->Draw("P"); // PLC PMC // PLC (Palette Line Color) and PMC (Palette Marker Color)
