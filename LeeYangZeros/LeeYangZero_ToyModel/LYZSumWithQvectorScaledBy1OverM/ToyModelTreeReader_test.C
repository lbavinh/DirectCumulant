...
void ToyModelTreeReader_test(TString inputFileName = "ToyModel.root", TString outputFileName = "test.root", TString inputFileHist="", Bool_t bFirstRun = 1)
{
        ...
        for (int rbin = 0; rbin < rbins; rbin++)
        {
          for (int thetabin = 0; thetabin < thetabins; thetabin++)
          {
            ...
            if (bUseProduct)
            {
              ...
              prReGthetaProduct[icent][thetabin]->Fill(r[rbin], genfunP[rbin][thetabin].Re()/mult); // mult
              prImGthetaProduct[icent][thetabin]->Fill(r[rbin], genfunP[rbin][thetabin].Im()/mult); // mult
              if (thetabin==0 && jentry==1) cout << genfunP[rbin][thetabin].Re() << endl;    
            }
          }
        }
