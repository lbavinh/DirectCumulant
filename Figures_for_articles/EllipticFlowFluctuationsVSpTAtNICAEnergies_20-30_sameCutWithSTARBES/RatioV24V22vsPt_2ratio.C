void RatioV24V22vsPt_2ratio(){
  ...
  const float leg_coordinate[4]={0.2,0.05,0.5,0.25}; //  0.05,0.7,0.35,0.99
  const float labelSize = 0.095;
  const float titleSize = 0.115;
  ...
  TString legendEntries[]={"STAR", "Model", "Model, #eta-gap"};
  ...
  TGraphErrors *grRatioV2[nenergy][nmodel][nmethod][ncent];
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int imod=0;imod<nmodel;imod++){
      ...
      input[ien][imod] = new TFile(Form("./Data/v2_%s_%sGeV_samecutwithSTARBES.root",model[imod].Data(),energy[ien].Data()),"read");
      ...
      for (int imeth=0;imeth<nmethod;imeth++){ // v22, v24, v2(eta-sub), v22(eta-gap)
        for (int icent=0;icent<ncent;icent++){
          ...
          // for (int i=10;i<grV2[ien][imod][imeth][icent]->GetN();i++){
          //   grV2[ien][imod][imeth][icent] -> RemovePoint(i);
          // }
        }
      }
    }
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int imod=0;imod<nmodel;imod++){
      for (int imeth=0; imeth<nmethod; imeth++){
        for (int icent=0;icent<ncent;icent++){
          if      (ien==2 && imod!=4)            grRatioV2[ien][imod][imeth][icent] = new TGraphErrors(2,xx,yy,xx,yy); // at 19.6 GeV only have vHLLE+UrQMD
          else if (ien==3 && imod==1)            grRatioV2[ien][imod][imeth][icent] = new TGraphErrors(2,xx,yy,xx,yy);  // at 27 GeV only SMASH is missing
          else if (ien==4 && imod!=2 && imod!=4) grRatioV2[ien][imod][imeth][icent] = new TGraphErrors(2,xx,yy,xx,yy);  // at 39 GeV ony AMPT15 & vHLLE+UrQMD
          else{
            std::vector<Double_t> vRatioV22GappedV24, vRatioV22GappedV24Err;
            for (int i=0;i<nbins[ien][imod][imeth][icent];i++){
              Double_t ratio = vy_gr[ien][imod][1][icent][i]/vy_gr[ien][imod][imeth][icent][i];
              Double_t ratioErr = ratio*(TMath::Sqrt(TMath::Power(ey_gr[ien][imod][imeth][icent][i]/vy_gr[ien][imod][imeth][icent][i],2)+TMath::Power(ey_gr[ien][imod][1][icent][i]/vy_gr[ien][imod][1][icent][i],2)));
              vRatioV22GappedV24.push_back(ratio);
              vRatioV22GappedV24Err.push_back(ratioErr);
            }
            grRatioV2[ien][imod][imeth][icent] = new TGraphErrors(nbins[ien][imod][imeth][icent],vx_gr[ien][imod][imeth][icent],&vRatioV22GappedV24[0],ex_gr[ien][imod][imeth][icent],&vRatioV22GappedV24Err[0]);      
          }
        }
      }
    }
  ...
  for (int imod=0;imod<nmodel;imod++){
    ...
    for (int ien=0;ien<nenergy;ien++){
      // for (int imeth=0; imeth<nmethod; imeth++){
      for (int icent=0;icent<ncent;icent++){
        // grRatioV2[ien][imod][imeth][1] -> SetMarkerStyle(kFullCircle);
        // grRatioV2[ien][imod][imeth][1] -> SetMarkerColor(kRed+2);
        // grRatioV2[ien][imod][imeth][1] -> SetLineColor(kRed+2);

        // grRatioV2[ien][imod][imeth][3] -> SetMarkerStyle(kFullTriangleDown);
        // grRatioV2[ien][imod][imeth][3] -> SetMarkerColor(kBlue+3);
        // grRatioV2[ien][imod][imeth][3] -> SetLineColor(kBlue+3);

        // grRatioV2[ien][imod][imeth][0] -> SetMarkerStyle(kOpenTriangleUp);
        // grRatioV2[ien][imod][imeth][0] -> SetMarkerColor(kGreen+3);
        // grRatioV2[ien][imod][imeth][0] -> SetLineColor(kGreen+3);

        // grRatioV2[ien][imod][imeth][4] -> SetMarkerStyle(kOpenTriangleUp); // kOpenSquare
        // grRatioV2[ien][imod][imeth][4] -> SetMarkerColor(kYellow+3);
        // grRatioV2[ien][imod][imeth][4] -> SetLineColor(kYellow+3);

        // grRatioV2[ien][imod][imeth][2] -> SetMarkerStyle(kOpenCircle);
        // grRatioV2[ien][imod][imeth][2] -> SetMarkerColor(kBlack);
        // grRatioV2[ien][imod][imeth][2] -> SetLineColor(kBlack);

        grRatioV2[ien][imod][0][icent] -> SetMarkerStyle(kOpenCircle);
        grRatioV2[ien][imod][0][icent] -> SetMarkerColor(kBlack);
        grRatioV2[ien][imod][0][icent] -> SetLineColor(kBlack);

        grRatioV2[ien][imod][3][icent] -> SetMarkerStyle(kFullTriangleDown);
        grRatioV2[ien][imod][3][icent] -> SetMarkerColor(kBlue+3);
        grRatioV2[ien][imod][3][icent] -> SetLineColor(kBlue+3);
      }
    }
  }
  ...
  for (int imod=0;imod<nmodel;imod++){
    for (int ien=0;ien<nenergy;ien++){
      for (int imeth=0; imeth<nmethod; imeth++){
      for (int icent=0;icent<ncent;icent++){
        grRatioV2[ien][imod][imeth][icent]->SetMarkerSize(markerSize);
      }
      }
    }
  }

  TString energySTAR[]={"7.7","11.5","19.6","27","39"};
  TString energySTARLegend[]={"7.7 GeV","11.5 GeV","19.6 GeV","27 GeV","39 GeV"};
  ...
  for (int iener=0;iener<nenergy;iener++){
    ...
    grRatioV2STAR[iener] -> SetMarkerSize(2); // markerSize+1 
  ...
  TPaveLabel* title = new TPaveLabel(0.1,0.97,0.9,0.985,"Au+Au, 20-30%, |#Delta#eta|<1.0, Ch. hadrons");
  ...
  for (int imod=0;imod<nmodel;imod++){
    ...
    else if (ien==1 && imod==2) h[imod][ien] = new TH2F(Form("pad_%i_%i",imod,ien),";p_{T} (GeV/c);",1,minpt,maxpt,1,minV2Ratio,maxV2Ratio);
    ...
    if (ien==0) tex.DrawLatex(minpt+0.1,maxV2Ratio*0.98,padName[imod].Data());
    if (ien==1) tex.DrawLatex(minpt+0.1,maxV2Ratio*0.98,padName[imod+5].Data());
    if (ien==0) tex.DrawLatex(minpt+0.5,maxV2Ratio*0.98,Form("%s",padFancyName[imod].Data())); // #sqrt{s_{NN}}=
    if (imod==0) tex.DrawLatex(minpt+0.3,minV2Ratio+0.1,Form("#sqrt{s_{NN}}=%s GeV",energy[ien].Data()));
    ...
    if (imod==1 && ien==0) {
      ...
      leg_pt->AddEntry(grRatioV2[ien][imod][0][2],legendEntries[1].Data(),"p");
      leg_pt->AddEntry(grRatioV2[ien][imod][3][2],legendEntries[2].Data(),"p");
      leg_pt->AddEntry(grRatioV2STAR[ien],legendEntries[0].Data(),"p");
      leg_pt->Draw();
    }
    TLine lineOne;
    ...
    for (int icent=0;icent<ncent;icent++){
        ...
        if (icent!=2) continue;
        grRatioV2[ien][imod][0][icent]->Draw("P");
        grRatioV2[ien][imod][3][icent]->Draw("P");
      
    }
  
  }
  }
  can->SaveAs("Figure_Ratiov24v22_vs_pt_20-30_2ratio_samecutwithSTARBES.pdf");
  // gROOT->SetStyle("Pub");
  can->SaveAs("Figure_Ratiov24v22_vs_pt_20-30_2ratio_samecutwithSTARBES.png");
