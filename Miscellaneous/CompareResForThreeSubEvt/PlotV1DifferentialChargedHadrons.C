...
void PlotV1DifferentialChargedHadrons()
{
  TString energy = {"7.7"};
  ...
  const double maxRatio = 1.55;
  const double minRatio = 0.45;
  const double maxX = 1.6;
  const double minX = -0.08;
  const double maxY = 0.11;
  const double minY = -0.11;
  ...
  bool bUseProduct = 1;
  Int_t nmethod = 2;
  TString title[]={"#it{v}_{1}{#Psi_{1,FHCal full}} |#Delta#eta|>0.5","#it{v}_{1}{#Psi_{1,FHCal half}} |#Delta#eta|>2.2"};
  TGraphErrors *graph[ncent][nmethod];
  TProfile *prV1_FHCalEP[ncent];
  TProfile *prV1_FHCalHalfEP[ncent];
  TFile *firun1 = new TFile("v1_eta_0.2_1.5_FHCalEP.root","read");
  for (int ic = 0; ic < ncent; ic++)
  {
    prV1_FHCalEP[ic] = (TProfile *) firun1->Get(Form("prV1_FHCalEP_%i",ic));
    prV1_FHCalHalfEP[ic] = (TProfile *) firun1->Get(Form("prV1_FHCalHalfEP_%i",ic));
    graph[ic][0] = Converter(prV1_FHCalEP[ic]);
    graph[ic][1] = Converter(prV1_FHCalHalfEP[ic]);  
    ...
    graph[ic][i]->GetYaxis()->SetTitle("#it{v}_{1}");
  ...
  TPaveLabel* label = new TPaveLabel(0.1,0.95,0.9,0.99,Form("UrQMD, Au+Au at #sqrt{s_{NN}} = %s GeV, Charged Hadrons, 0.2<|#eta|<1.5",energy.Data()));
  ...
  for (int ic=firstCentToDraw; ic<numberOfCentToDraw+firstCentToDraw; ic++)
  {
    ...
    {
    TLegend *leg1 = new TLegend(0.2,0.05,0.7,0.22);
    ...
    for (int i=0; i<vGr[ic].size(); i++)
    {
      vGr[ic].at(i)->SetTitle(";#it{p}_{T}, GeV/c;#it{v}_{1}");
    }
    TLine lineZero;
  ...
  can->SaveAs(Form("v1pt_FHCalEPFull_vs_FHCalEPHalf.pdf"));
  gROOT->SetStyle("Pub");
  can->SaveAs(Form("v1pt_FHCalEPFull_vs_FHCalEPHalf.png"));
