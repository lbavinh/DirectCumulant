    ...
    for (int iEv = 0; iEv < n_entries; iEv++)
    {
        ...
        for (int iTrk = 0; iTrk < nh; iTrk++)
        {
            ...
            for (int thetabin = 0; thetabin < thetabins; ++thetabin)
            {
                ...
                Qtheta[thetabin] += TComplex(cos(2. * angle), sin(2. * angle));
            }
            ...
            for (int rbin = 0; rbin < rbins; ++rbin)
            {
                ...
                for (int thetabin = 0; thetabin < thetabins; ++thetabin)
                {
                    float theta_width = (2.0 * TMath::Pi()) / thetabins;
                    float theta = thetabin * theta_width;
                    float angle = phi - theta;
        ...
        for (int rbin = 0; rbin < rbins; ++rbin)
        {
            ...
            for (int thetabin = 0; thetabin < thetabins; ++thetabin)
            {
                genfunS[rbin][thetabin] = TComplex::Exp(r * Qtheta[thetabin]); // generating function from Q-vectors
                LeeYangHistosS[fcent][thetabin]->Fill(r, genfunS[rbin][thetabin].Rho());
                LeeYangHistosP[fcent][thetabin]->Fill(r, genfunP[rbin][thetabin].Rho());
            }
        }
