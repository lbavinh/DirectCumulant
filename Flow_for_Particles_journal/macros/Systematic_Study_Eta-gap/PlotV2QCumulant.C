...
void CalStatErrCent1040(TString sEta_gap,double v2eDif1040[nmethod][npid][npt]){

  TFile *inFile = new TFile(Form("./QCumulant_eta_gap_%s.root",sEta_gap.Data()),"read");
...
vector<TGraphErrors*> PlotV2QCumulant(TString sEta_gap, TString model = "AMPT", TString energy = "7.7GeV"){
  ...
  TString inputFileName = Form("./QCumulant_eta_gap_%s.root",sEta_gap.Data());
  CalStatErrCent1040(sEta_gap,v2eDif1040);
  TFile *inFile = new TFile(inputFileName.Data(),"read");
  ...
  inFile->Close();
  return vgrv2cent_chargedHardons;
  // for (int icent=0; icent<ncent; icent++){ // loop over centrality classes
  //   delete hv22[icent];
  //   delete hv24[icent];
  //   delete hcov24[icent];
