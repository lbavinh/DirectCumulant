...
void Performance_QC()
{
  ...
  TString input[nmethod] = {"v2_FirstRun_vHLLEUrQMD_11.5_Model.root_full.root",
                            "v2_FirstRun_vHLLEUrQMD_11.5_Reco_Nhits_16_DCA_3sigma_pt_weight.root_full.root"};
  ...
  TString title[]={"Model","Reco, 3#sigma_{DCA}"};// MC GEANT4 Reco
  ...
  for (int i = 0; i < npid; i++)
  {
    ...
    {
      graph[i][j] = (TGraphErrors*) fi[j]->Get(Form("gr_cent3_1_%i",pidcode[i]));
    }
  }
  ...
  TCanvas *can = (TCanvas*)DrawTGraph(vGr[ic],"20-30%",0.89, 1.11, 0, 3.0, -0.005, 0.25,
                                      // 0.65, 0.05, 0.9, 0.5,
                                      0.2, 0.65, 0.4, 0.88,
                                      "vHLLE+UrQMD, Au+Au", Form("#sqrt{s_{NN}} = 11.5 GeV, %s, |#eta|<1.5", pidNameFancy[ic].Data()),1,"Ratio to Model"); // #pi^{#pm}
  can->SetName(Form("10-40%%"));
  can->SaveAs(Form("Performance_v2_4QC_2030_Model_vs_Reco_Nhits_16_%s_DCA_sigma_3.pdf",pidName[ic].Data()));
  }
