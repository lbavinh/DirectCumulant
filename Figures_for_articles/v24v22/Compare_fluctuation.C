void Compare_fluctuation(){
  ...
  const float leg_coordinate[4]={0.3,0.25,0.6,0.5}; //  0.05,0.7,0.35,0.99
  const float labelSize = 0.07;
  const float titleSize = 0.085;
  ...
  for (int iener=0;iener<4;iener++){
    ...
    for (int ien=0;ien<4;ien++){
      grRatioV2STAR[ien]->SetMarkerSize(1.8);
    }
  ...
  h[0]->GetYaxis()->SetTitleOffset(0.95);
  h[0]->Draw();
  ...
  for (int ipad=1;ipad<4;ipad++){
    can->cd(ipad+1);
    if (ipad==1) h[ipad] = new TH2F(Form("pad_%i",ipad+1),Form(";;"),1,mincent,maxcent,1,minV2Ratio,maxV2Ratio);
    else if (ipad==2) h[ipad] = new TH2F(Form("pad_%i",ipad+1),Form(";%s         ;v_{2}",xAxisName.Data()),1,mincent,maxcent,1,minV2Ratio,maxV2Ratio);
    ...
    if (ipad==1) {
      ...
      TLegend *leg_pt = new TLegend(leg_coordinate[0],leg_coordinate[1],leg_coordinate[2],leg_coordinate[3]-0.08);
      ...
      tex.DrawLatex(mincent+10,maxV2Ratio*0.98-0.12,Form("#sqrt{s_{NN}}=%s GeV",energy[ipad-1].Data()));
      TLegend *leg_pt = new TLegend(leg_coordinate[0],leg_coordinate[1],leg_coordinate[2],leg_coordinate[3]-0.15);
      leg_pt->SetBorderSize(0);
      leg_pt->SetTextFont(42);
      leg_pt->SetTextSize(labelSize-0.01);
  ...
  can->SaveAs("Figure_2_Ratiov24v22_v2cent.pdf");
  gROOT->SetStyle("Pub");
  can->SaveAs("Figure_2_Ratiov24v22_v2cent.png");
