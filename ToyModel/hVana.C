...
static const int npid = 1; // charged hadrons
...
static const float eta_cut =  2.0; // min pt
static const float eta_gap = 0.2; // min pt
...
TProfile *hv2MCpt[ncent][npt][npid]; // profile v2pt from MC toy
...
void hVana::Booking(TString outFile)
{
  ...
  for (int icent=0;icent<ncent;icent++){ // loop over centrality classes
    hv2MC[icent] = new TProfile(Form("hv2MC_%i",icent),Form("hv2MC_%i",icent),1,0.,1.);
    ...
    for (int id=0;id<npid;id++){
      ...
      for (int ipt = 0; ipt < npt; ipt++){ // loop over pt bin
        hv2MCpt[icent][ipt][id] = new TProfile(Form("hv2MCpt_%i_%i_%i",icent,ipt,id),Form("hv2MCpt_%i_%i_%i",icent,ipt,id),1,0.,1.);
...
void hVana::Ana_event(){
  int fcent = -1;
  for (int i = 0; i < ncent; i++) if (CentB(b) == bin_cent[i]) fcent = i;
  if (fcent < 0) return;
  hMult->Fill(nh);
  hRP->Fill(rp);
  hBimp->Fill(b);
  ...
  for (int iTrk = 0; iTrk < nh; iTrk++){ // track loop
    ...
    if (bFlow[iTrk]) {
      hv2MCpt[fcent][ipt][0]->Fill(0.5, v2, 1); // Calculate differential v2 from MC toy
    ...
    for (int ieta=0;ieta<neta;ieta++){
      for (int ipt = 0; ipt < npt; ipt++){ // <2'>
        for (int id=0;id<npid;id++){
          ...
          if (ieta==0) hcounter[fcent][ipt][id]->Fill(1.5,1,mpGap[ieta][ipt][id]);
...
void loop_a_list_of_trees()
{
  ...
  ana->Booking("/weekly/lbavinh/lbavinh/ToyModel/ToyModel.root");

  // ifstream ifile("/mnt/pool/2/lbavinh/EventGenerator/OUT/Pure/runlist.list"); // tree runlist
  ifstream ifile("/weekly/lbavinh/lbavinh/ToyModel/runlist.list"); // tree runlist
  char filename[200];
  int nfiles = 1;
