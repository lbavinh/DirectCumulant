void RatioV24V22vsCentralityPID(){
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int imod=0;imod<nmodel;imod++){
      ...
      for (int imeth=0;imeth<nmethod;imeth++){ // v22, v24, v2(eta-sub), v22(eta-gap)
        ...
        grV2[ien][imod][imeth][0] = (TGraphErrors*)input[ien][imod]->Get(Form("grRF_%i_1",imeth)); // Pion+
        grV2[ien][imod][imeth][1] = (TGraphErrors*)input[ien][imod]->Get(Form("grRF_%i_2",imeth)); // Kaon+
        grV2[ien][imod][imeth][2] = (TGraphErrors*)input[ien][imod]->Get(Form("grRF_%i_3",imeth)); // Proton
        grV2[ien][imod][imeth][3] = (TGraphErrors*)input[ien][imod]->Get(Form("grRF_%i_5",imeth)); // Pion-
        grV2[ien][imod][imeth][4] = (TGraphErrors*)input[ien][imod]->Get(Form("grRF_%i_6",imeth)); // Kaon-
  ...
  can->SaveAs("Figure_Ratiov24v22_vs_cent_PID.pdf");
  gROOT->SetStyle("Pub");
  can->SaveAs("Figure_Ratiov24v22_vs_cent_PID.png");
