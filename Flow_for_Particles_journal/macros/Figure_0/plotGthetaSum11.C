...
void plotGthetaSum11()
{
  TFile *fi = new TFile("../FirstRun_11.5.root","read");
  const int ncent = 9;
  const int rbins = 500;
  ...
  tex.DrawLatex(0.36,0.2,"(b)");
  tex.DrawLatex(0.21,0.02,"#splitline{AMPT SM, #it{#sigma}_{p}=1.5 mb}{Au+Au at #sqrt{#it{s}_{NN}} = 11.5 GeV}");

  gPad->SetLogy();
  gStyle->SetOptStat(0);
  c.SaveAs("Gtheta115.pdf");
  c.SaveAs("Gtheta115.png");
}
