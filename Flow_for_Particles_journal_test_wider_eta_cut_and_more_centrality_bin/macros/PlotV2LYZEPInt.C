...
void PlotV2LYZEPInt(TString inputFileName1 = input1, TString inputFileName3 = input3)
{
  bool bUseProduct = 1;
  bool bDebug = 1;
  TGraphErrors *grV2IntLYZ[3];
  TString label = Form("AMPT, #sigma_{p}=1.5 mb, Au+Au, #sqrt{s_{NN}}= %s GeV",energy.Data());
  TFile *fi3 = new TFile(inputFileName3.Data(),"read");
  TProfile3D *prV2LYZEP3D = (TProfile3D *)fi3->Get("prV2LYZEP");
  TProfile *prV2LYZEPInt = PlotV2LYZEPIntegrated(prV2LYZEP3D,0.2,3.0,1.5);
  grV2IntLYZ[2] = Converter(prV2LYZEPInt);
  grV2IntLYZ[2]->SetTitle("v_{2}{LYZ, EP};Centrality, %;v_{2}");
  grV2IntLYZ[2]->SetMarkerStyle(30);
  grV2IntLYZ[2]->SetMarkerColor(kBlue+2);
  grV2IntLYZ[2]->SetLineColor(kBlue+2);
  // grV2IntLYZ[2]->GetYaxis()->SetRangeUser(-0.01,0.1);
  ...
  TGraphErrors *grRatioLYZEPToSum = Ratio(grV2IntLYZ[2],grV2IntLYZ[0]);
  TString legEntries[3] = {"v_{2}{LYZ, Sum}","v_{2}{LYZ, Prod.}","v_{2}{LYZ, EP}"};
  ...
  grRatioProductToSum->SetMarkerColor(kBlack);
  grRatioProductToSum->SetLineColor(kBlack);

  grRatioLYZEPToSum->SetTitle(Form(";Centrality, %%;%s/%s",legEntries[2].Data(),legEntries[0].Data()));
  grRatioLYZEPToSum->GetYaxis()->SetRangeUser(0.79,1.19);
  grRatioLYZEPToSum->SetMarkerColor(kBlue+2);
  grRatioLYZEPToSum->SetLineColor(kBlue+2);
  ...
  c.Divide(2,1);
  c.cd(1);
  grV2IntLYZ[0]->Draw("AP");
  grV2IntLYZ[1]->Draw("P");
  grV2IntLYZ[2]->Draw("P");
  TLegend *leg = new TLegend(0.6,0.2,0.85,0.45);
  
  leg->SetBorderSize(0);
  for (int i=0;i<3;i++) leg->AddEntry(grV2IntLYZ[i],legEntries[i].Data(),"p");
  leg->Draw();
  c.cd(2);
  TLine lineOne;
  lineOne.SetLineStyle(2);
  grRatioProductToSum->Draw("AP");
  grRatioLYZEPToSum->Draw("P");
  lineOne.DrawLine(0,1,80,1);
  c.SaveAs(Form("IntFlowLYZEP_%s.pdf",energy.Data()));
