...
void plotGthetaSum7_testNewWeight_AMPT08_forPresentation()
{
  TFile *fi = new TFile("../FirstRun_AMPT08_7.7_new.root","read");
  const int ncent = 9;
  const int rbins = 500;
  ...
  double rmax = 250;
  int thetaDraw = 2;
  int edge[ncent] = {1200,1200,790,2500,490,460,430,45,40};// 550
  TH1F *hGtheta[ncent];
  for (int ic=0; ic<ncent; ic++)
  {
  ...
  for (int ic=0; ic<ncent; ic++)
  {
    ...
    hGtheta[ic] -> GetXaxis()->SetRangeUser(0.,250);
  }
  gStyle->SetPalette(kDarkRainBow);
  // hGthetaSum[0][thetaDraw] -> GetYaxis()->SetRangeUser(0,1);
  
  hGtheta[3]->SetTitle(";#it{r};|#it{G}^{#it{#theta}}(i#it{r})|^{2}");
  hGtheta[3]->GetXaxis()->SetTitleSize(0.06);
  hGtheta[3]->GetXaxis()->SetLabelSize(0.05);
  hGtheta[3]->GetYaxis()->SetTitleSize(0.06);
  hGtheta[3]->GetYaxis()->SetLabelSize(0.05);
  hGtheta[3]->GetYaxis()->SetTitleFont(textFont);
  hGtheta[3]->GetYaxis()->SetLabelFont(textFont);
  hGtheta[3]->GetXaxis()->SetTitleFont(textFont);
  hGtheta[3]->GetXaxis()->SetLabelFont(textFont);
  hGtheta[3]->Draw("L PLC PMC");
  hGtheta[3]->GetXaxis()->SetLimits(0,rmax);
  ...
  // tex.DrawLatex(90,0.2,"(a)");
  tex.DrawLatex(55,0.01,"#splitline{AMPT SM, #it{#sigma}_{p}=0.8 mb}{#splitline{Au+Au at #sqrt{#it{s}_{NN}} = 7.7 GeV}{20-30%, #it{#theta} = #pi/5}}");
  
  gPad->SetLogy();
  gStyle->SetOptStat(0);
  c.SaveAs("Gtheta77_testNewWeight_AMPT08_forPresentation.pdf");
  c.SaveAs("Gtheta77_testNewWeight_AMPT08_forPresentation.png");
}
