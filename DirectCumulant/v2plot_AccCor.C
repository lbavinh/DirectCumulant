...
void v2plot_AccCor(){
  // Input 
  TFile *inFile;
  inFile = new TFile("./ROOTFile/acceptance.root","read");
  TH1F *hBimp = (TH1F*)inFile->Get("hBimp");
  cout << "Number of events = " << hBimp -> GetEntries() << endl;
  ...
  TGraphErrors *grDifFlAC[2][ncent], *grRefFlAC[ncent]; // 2 = {2QC,4QC}
  TMultiGraph *mgRefFlAC[ncent], *mgDifFlAC[ncent];
  ...
  for (int icent=0; icent<ncent; icent++){
    for (int ipt=0; ipt<npt; ipt++){
      // pt[icent][ipt] = hPT[ipt] -> GetBinContent(1+icent);
  ...
  for (int icent=0; icent<ncent; icent++){ // loop over centrality classes
    ...
    grRefFl[icent]->SetMarkerStyle(24);
    grRefFl[icent]->SetMarkerSize(1.3);
    grRefFl[icent]->SetDrawOption("P");

    grRefFlAC[icent] = new TGraphErrors(3,x,v2AC[icent],ex,ev2[icent]);
    grRefFlAC[icent]->SetMarkerColor(kRed);
    grRefFlAC[icent]->SetMarkerStyle(20);
    grRefFlAC[icent]->SetMarkerSize(1.3);
    grRefFlAC[icent]->SetDrawOption("P");
    ...
    mgRefFl[icent] -> Add(grRefFlAC[icent]);
    ...
    grDifFl[1][icent] -> SetMarkerStyle(20+4);

    grDifFl[2][icent] = new TGraphErrors(npt,pt[icent],v24dif,ept[icent],v24difE);
    grDifFl[2][icent] -> SetMarkerColor(kAzure+2);
    grDifFl[2][icent] -> SetMarkerStyle(22+4);
    mgDifFl[icent] = new TMultiGraph();

    // AC
    grDifFlAC[0][icent] = new TGraphErrors(npt,pt[icent],v22difAC,ept[icent],v22difE);
    grDifFlAC[0][icent] -> SetMarkerColor(kGreen+1);
    grDifFlAC[0][icent] -> SetMarkerStyle(20);

    grDifFlAC[1][icent] = new TGraphErrors(npt,pt[icent],v24difAC,ept[icent],v24difE);
    grDifFlAC[1][icent] -> SetMarkerColor(kAzure+2);
    grDifFlAC[1][icent] -> SetMarkerStyle(22);
    mgDifFlAC[icent] = new TMultiGraph();
    ...
    for (int i=0; i<2; i++){
      grDifFlAC[i][icent] -> SetMarkerSize(1.3);
      grDifFlAC[i][icent] -> SetDrawOption("P");
      mgDifFlAC[icent] -> Add(grDifFlAC[i][icent]);
    }    
  } // end of loop over centrality classes
  ...
  for(int icent=0; icent<6; icent++){
    ...
    double ymin1 = TMath::MinElement(3,v2[icent])*0.98;
    double ymax1 = TMath::MaxElement(3,v2[icent])*1.02;
    h2[icent] = new TH2F("","",3,0,3,15,0.02,0.17);
    c2 -> cd(icent+1);
    h2[icent]->SetYTitle("v_{n}");
    h2[icent]->SetCanExtend(TH1::kAllAxes);
    TAxis* a = h2[icent] -> GetXaxis();
    for (int j=0; j<3; j++) h2[icent]->Fill(ch[j],(ymin1+ymax1)/2.,1);
    ...
    for (int j=0; j<3; j++) h3[icent]->Fill(ch[j],0.02,1);
  ...
  for (int i=0;i<ncent;i++){
    ...
    h4[i] = new TH2F("","",3,0,3,10,ymin2,ymax2);
    h4[i]->SetYTitle("v_{n}");
    h4[i]->SetCanExtend(TH1::kAllAxes);
    
    TAxis* a = h4[i] -> GetXaxis();
    for (int j=0; j<3; j++) h4[i]->Fill(ch[j],(ymin2+ymax2)/2.,1);
    ...
    text[i] -> Draw();
    sprintf(hname,"./Graphics/acceptance/Cent%i-%i%%.png",i*10,(i+1)*10);
    c[i] -> SaveAs(hname);
  }
