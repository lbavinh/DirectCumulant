void Compare_fluctuation_for_presentation(){
  ...
  for (int ien=0;ien<nenergy;ien++){
    ...
    grRatioV2[ien][3] -> SetMarkerColor(kMagenta+3);
    grRatioV2[ien][3] -> SetLineColor(kMagenta+3);
    grRatioV2[ien][3] -> SetMarkerStyle(kFullTriangleDown); // kOpenTriangleDown

    grRatioV2[ien][4] -> SetMarkerColor(kBlack);
    grRatioV2[ien][4] -> SetLineColor(kBlack);
    grRatioV2[ien][4] -> SetMarkerStyle(kOpenSquare);

    grRatioV2[ien][5] -> SetMarkerColor(kMagenta+1);
    grRatioV2[ien][5] -> SetLineColor(kMagenta+1);
  ...
  // gROOT->SetStyle("Pub");
  gStyle->SetErrorX(0);
  TCanvas *can = new TCanvas("can","can",200,10,1500,1000);
  can->SetLeftMargin(0.18);
  can->SetRightMargin(0.01);
  can->SetBottomMargin(0.18);
  can->SetTopMargin(0.12);
  TPaveLabel* title = new TPaveLabel(0.1,0.93,0.9,0.95,"Au+Au, Charged hadrons, 0.2 <#it{p}_{T}< 3.0 GeV/c");
  ...
  for (int ipad=0;ipad<nenergy;ipad++){
    ...
    if (ipad==0) h[ipad]->GetXaxis()->SetLabelSize(labelSize-0.01);
    ...
    h[ipad]->GetXaxis()->SetTitleOffset(0.7);
    // h[ipad]->GetXaxis()->SetLabelOffset(0.005);
    if (ipad==0) h[ipad]->GetXaxis()->SetLabelOffset(0.012);
    ...
    for (int imod=nmodel-1;imod>=0;imod--){
      ...
      if (ipad==0 && imod!=1 && imod!=2) continue;
  ...
  gROOT->SetStyle("Pub");
  can->SaveAs("Ratiov24v22_v2cent_45_115GeV_models_vs_STAR_report.png");
