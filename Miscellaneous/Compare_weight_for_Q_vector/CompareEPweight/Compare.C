...
void Compare()
{
  TString inputName[3] = {"./SecondRun_vHLLEUrQMD_11.5_Model_EP_1_weight.root",
                          "./SecondRun_vHLLEUrQMD_11.5_Model.root"};
  ...
  for (int j=0; j<npid; j++)
  {
    auto pr = (TProfile2D*) fi[1]->Get(Form("ETASUBEP/prV2EtaSubvsPt_pid%i",pidCode[j]));
    TProfile *tmp = PlotV2vsPt(pr,10,40);
  ...
  for (int id = 0; id < npid; id++)
  {
    ...
    can->SaveAs(Form("Compare_EP_%i%i_Model_%s.pdf",centlow,centhigh,pidName[id].Data()));
  }
