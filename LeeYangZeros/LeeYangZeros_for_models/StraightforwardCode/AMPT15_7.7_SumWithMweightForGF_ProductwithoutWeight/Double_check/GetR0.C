...
TH1F* FillHistGtheta(TProfile *const &prReGtheta, TProfile *const &prImGtheta)
{
  ...
  TH1F* hGtheta = new TH1F("","",iNbins,xMin,xMax); // Form("hist_%s",prReGtheta->GetName())
...
void GetR0(TString inputFileName = "FirstRun.root")
{
  bool bDebug = 1;
  ...
  TH1F *hGthetaPro[ncent][thetabins];
  ...
  for (int ic = 0; ic < ncent; ic++)
  {
    ...
    for (int it = 0; it < thetabins; it++)
    {
      TH1F *hGtheta = NULL;
      if (bUseProduct) {
        hGtheta = FillHistGtheta(prReGthetaProduct[ic][it], prImGthetaProduct[ic][it]);
        if (bDebug) hGthetaPro[ic][it] = FillHistGtheta(prReGthetaProduct[ic][it], prImGthetaProduct[ic][it]);
      }
      else {
  ...
  if (bDebug)
  {
    ...
    if (!bUseProduct) {
      hGthetaSum[1][1]->Draw();
      cout << GetR0(hGthetaSum[cent][theta]) << endl;
      gPad->SetLogy();
    }
    else {
      hGthetaPro[cent][theta]->Draw();
      cout << GetR0(hGthetaPro[cent][theta]) << endl;
    }
  }
