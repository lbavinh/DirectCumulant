void Compare_fluctuation(){
  ...
  const TString leg_header[nenergy] = {"4.5 GeV","7.7 GeV","#splitline{Au+Au@#sqrt{s_{NN}}=11.5 GeV}{Charged Hadrons, 0.2<p_{T}<3.0 GeV/c}"};
  ...
  for (int ien=0;ien<nenergy;ien++){
    ...
    grRatioV2[ien][2] -> SetMarkerStyle(kOpenCircle);
  ...
  //=================================================================================
  can->cd(1);
  TH2F *h1 = new TH2F("h1",";;v_{2}{4}/v_{2}{2,|#Delta#eta|>0.1}",1,minpt,maxpt,1,minV2Ratio,maxV2Ratio);
  ...
  // //=================================================================================
  can->cd(3);
  TH2F *h3 = new TH2F("h3",";;v_{2}{4}/v_{2}{2,|#Delta#eta|>0.1}",1,minpt,maxpt,1,minV2Ratio,maxV2Ratio);
  ...
  h3->Draw();
  TLegend *leg_pt3 = new TLegend(leg_coordinate[0]+0.8,leg_coordinate[1]-0.05,leg_coordinate[2]-0.3,leg_coordinate[3]-0.05);
  leg_pt3->SetBorderSize(0);
  leg_pt3->SetHeader(leg_header[2].Data(),"C");
  leg_pt3->SetTextSize(0.05);
  leg_pt3->SetTextAlign(11);
  for (int imod=nmodel-1;imod>=0;imod--){
  // for (int imod=0;imod<nmodel;imod++){
  ...
  can->SaveAs("ratiov2cent_vsSTAR_3ener_v24_v22gap_merge.png");
