void FlowFluctuations(){
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int imod=0;imod<nmodel;imod++){
      ...
      for (int imeth=0;imeth<nmethod;imeth++){ // v22, v24, v2(eta-sub), v22(eta-gap)
        ...
        grV2[ien][imod][imeth][0] = (TGraphErrors*)input[ien][imod]->Get(Form("gr_cent10-40_%i_1",imeth)); // Pion+
        grV2[ien][imod][imeth][1] = (TGraphErrors*)input[ien][imod]->Get(Form("gr_cent10-40_%i_2",imeth)); // Kaon+
        grV2[ien][imod][imeth][2] = (TGraphErrors*)input[ien][imod]->Get(Form("gr_cent10-40_%i_3",imeth)); // Proton
        grV2[ien][imod][imeth][3] = (TGraphErrors*)input[ien][imod]->Get(Form("gr_cent10-40_%i_5",imeth)); // Pion-
        grV2[ien][imod][imeth][4] = (TGraphErrors*)input[ien][imod]->Get(Form("gr_cent10-40_%i_6",imeth)); // Kaon-
  ...
  for (int imod=0;imod<nmodel;imod++){
    ...
    for (int ien=0;ien<nenergy;ien++){
      ...
      grRatioV2[ien][imod][3] -> SetMarkerStyle(kOpenCircle);
      grRatioV2[ien][imod][1] -> SetMarkerStyle(kFullTriangleUp);
      grRatioV2[ien][imod][4] -> SetMarkerStyle(kOpenTriangleUp);
