void Compare_fluctuation(){
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int imod=1;imod<nmodel;imod++){
      if (ien==2 &&(imod==3||imod==4||imod==5)) continue;  
  ...
  for (int ipad=1;ipad<4;ipad++){
    ...
    for (int imod=nmodel-1;imod>=0;imod--){
      ...
      if (ipad==3) {
        if (imod==3||imod==4||imod==0||imod==5) continue;
      grRatioV2[2][imod]->Draw("P");
      }
    }

  }
  can->SaveAs("Figure_2_Ratiov24v22_v2cent_withvHLLE.pdf");
  gROOT->SetStyle("Pub");
  can->SaveAs("Figure_2_Ratiov24v22_v2cent_withvHLLE.png");
