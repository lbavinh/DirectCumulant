...
void plot(TString inFile)
{
  ...
  char text2[800];
  sprintf(text2,"#splitline{Nonflow: quadruplet (rate: 25%)}{#splitline{M=250#pm50}{#splitline{5#upoint10^{6} events}{RFP: %2.1f < p_{T} < %2.1f GeV/c}}}",minptRFP,maxptRFP);
...
void v2plot_nonflowQuadruplet(){
  plot("v2QC_5mil_nonflowQuadruplet.root");
}