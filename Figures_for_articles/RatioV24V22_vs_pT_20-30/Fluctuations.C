void Fluctuations(){
  ...
  const float leg_coordinate[4]={0.07,0.25,0.45,0.4}; //  0.05,0.7,0.35,0.99
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int imod=1;imod<nmodel;imod++){
      ...
      for (int imeth=0;imeth<nmethod;imeth++){ // v22, v24, v2(eta-sub), v22(eta-gap)
        grV2[ien][imod][imeth] = (TGraphErrors*)input[ien][imod]->Get(Form("gr_cent3_%i_0",imeth)); // "_0" - Charged Hadrons
  ...
  for (int ien=0;ien<nenergy;ien++){
    ...
    grRatioV2[ien][5] -> SetMarkerColor(kBlack);
    grRatioV2[ien][5] -> SetLineColor(kBlack);
    ...
    grRatioV2[ien][2] -> SetMarkerColor(kBlue+3);
    grRatioV2[ien][2] -> SetLineColor(kBlue+3);
    grRatioV2[ien][2] -> SetMarkerStyle(kOpenCross);

    grRatioV2[ien][1] -> SetMarkerColor(kYellow+2);
    grRatioV2[ien][1] -> SetLineColor(kYellow+2);
    grRatioV2[ien][1] -> SetMarkerStyle(kFullCrossX);
  ...
  TPaveLabel* title = new TPaveLabel(0.1,0.95,0.9,0.98,"Au+Au, 20-30%, Ch. hadrons");
  ...
  for (int ipad=0;ipad<nenergy;ipad++){
    ...
    tex.DrawLatex(minpt+0.5,maxV2Ratio*0.98,Form("%s GeV",energy[ipad].Data())); // #sqrt{s_{NN}}=
    if (ipad==2) {
