...
void plotE2vsNpart(TString model ="Glauber", TString energy = "200GeV") {
  
  TString inFileName = (TString) Form("../ROOTFile/%s_%s.root",model.Data(),energy.Data());
  TFile *outFile = new TFile(Form("./ecc_%s_%s_vsNpart.root",model.Data(),energy.Data()),"recreate");
  ...
  bool saveAsPNG = true;
  // Constant
  const int nNpartBin = 7; // 0-80 %
  const double bin_cent[nNpartBin] = {25,75,125,175,225,275,325};
  const double bin_centE[nNpartBin] = {0};
  const int nmethod = 3;
  const int ratioToMethod = 0;
  const double mincent = 0.;  // for v2 vs centrality
  const double maxcent = 350; // for v2 vs centrality
  ...
  // Histogram input
  TProfile *prEcc22 = (TProfile*)inFile->Get(Form("prEcc22vsNpart"));
  TProfile *prEcc24 = (TProfile*)inFile->Get(Form("prEcc24vsNpart"));
  TProfile *prCov24 = (TProfile*)inFile->Get(Form("prCov24vsNpart"));
  TProfile *prEcc2 = (TProfile *) inFile -> Get("prEcc2vsNpart");

  double ecc2[nmethod][nNpartBin], ecc2e[nmethod][nNpartBin];

  for (int icent=0;icent<nNpartBin;icent++){
    ...
    ecc2[2][icent] = prEcc2 -> GetBinContent(icent+1);
    ecc2e[2][icent] = prEcc2 -> GetBinError(icent+1);
  }
  for (int m=0; m<nmethod; m++){
    gr[m] = new TGraphErrors(nNpartBin,bin_cent,ecc2[m],bin_centE,ecc2e[m]);
  ...
  c->Divide(1,2,0,0);
  c->cd(1);
  TH2F *h = new TH2F("h",Form("Glauber, Au+Au at #sqrt{s_{NN}} = %s;N_{part};#epsilon_{2}", energy.Data()),nNpartBin,mincent,maxcent,1,minV2int,maxV2int);
  ...
  leg->Draw();
  c->cd(2);
  TH2F *hRatio = new TH2F("hRatio",Form(";N_{part};Ratio to #epsilon_{2}{2}"),nNpartBin,mincent,maxcent,1,0.3,1.4);
  ...
  c->SaveAs(Form("Eccentricity2_%s_%s_vsNpart.png",model.Data(),energy.Data()));
  delete prEcc22;
  delete prEcc24;
  delete prCov24;
  inFile->Close();
