...
void plotRelativeFluctuation_vs_Npart()
{
  ...
  const float maxY = 1.4;
  const float minY = 0.3;
  const float minX = 0.;
  const float maxX = 350.;

  double ratioRef[ncent] = {0.875,0.877,0.879,0.851,0.825,0.775,0.6};
  TGraphErrors *grRef = new TGraphErrors(ncent,bin_cent,ratioRef,bin_centE,bin_centE);
  grRef -> SetMarkerStyle(24);
  grRef -> SetMarkerSize(markerSize);
  grRef -> SetTitle("AMPT (Phys. Rev. C 94, 044915)");

  TFile *fiEcc = new TFile("../ROOTFile/Glauber_200GeV.root","read");
  ...
  for (int icent=0; icent<ncent; icent++){ // loop over centrality classes
    ...
    for (int imeth=0; imeth<nmethod; imeth++){
      ...
      ratioEcc2E[imeth][icent] = ratioEcc2[imeth][icent] * sqrt(pow(ecc2e_RF[imeth][icent]/ecc2_RF[imeth][icent],2) + pow(ecc2e_RF[ratioToMethod][icent]/ecc2_RF[ratioToMethod][icent],2));
  ...
  // ============================================================================================================================  
  grRatioEcc2[1] -> SetTitle("Glauber;N_{part};#epsilon_{2}{4}/#epsilon_{2}{2}");
  ...
  l->AddEntry(grRef,Form("%s",grRef->GetTitle()),"p");
  l->AddEntry(grRatioEcc2[1],Form("%s",grRatioEcc2[1]->GetTitle()),"p");
