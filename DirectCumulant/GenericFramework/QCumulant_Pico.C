...
void QCumulant_Pico(TString inputFileName, TString outputFileName)
{
  ...
  // if (!tree){
  //   std::cout << "picodst is not found in " << inputFileName.Data() << std::endl;
  //   treefile->Close();
  //   return;
  // }
  // std::cout << inputFileName.Data() << " is opened" << std::endl;
  ...
  for (int iEv=0; iEv<n_entries; iEv++)
  {
    ...
    for (int iTr=0; iTr<reco_mult; iTr++) { // track loop
      ...
      for(int iTr2=0;iTr2<reco_mult;iTr2++){
        if(iTr2==iTr) continue; // Get rid of autocorrelations
      auto recoTrack2 = (PicoDstRecoTrack*) recoTracks->UncheckedAt(iTr2);
      if (!recoTrack2) continue;
      auto mcTrack2 = (PicoDstMCTrack*) mcTracks->UncheckedAt(recoTrack2->GetMcId());
      if (!mcTrack2) continue;
      if (mcTrack2->GetMotherId() != -1) continue;
      if (recoTrack2->GetNhits() < Nhits_cut ) continue;
      float pt2  = recoTrack2->GetPt();
      float eta2 = recoTrack2->GetEta();
      float phi2 = recoTrack2->GetPhi();
      // if (abs(recoTrack->GetDCAx()) > DCAcut) continue; //трек не проходит по DCAx
      // if (abs(recoTrack->GetDCAy()) > DCAcut) continue; //трек не проходит по DCAy
      // if (abs(recoTrack->GetDCAz()) > DCAcut) continue; //трек не проходит по DCAz
      ...
      auto particle2 = (TParticlePDG*) TDatabasePDG::Instance()->GetParticle(mcTrack2->GetPdg());
