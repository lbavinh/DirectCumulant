...
void CalStatErrCent1040(double eV22cent1040[npid][npt], double eV24cent1040[npid][npt], double eV2EPcent1040[npid][npt], double eV22Gapcent1040[npid][npt]){
...
void v2plot_differential_flow(){
  double eV22cent1040[npid][npt], eV24cent1040[npid][npt], eV2EPcent1040[npid][npt], eV22Gapcent1040[npid][npt];
  CalStatErrCent1040(eV22cent1040,eV24cent1040,eV2EPcent1040,eV22Gapcent1040);
...
void v2plot_integrated_flow_for_PID(){ 
  ...
  for (int icent=0;icent<ncent;icent++){
    for (int id=0;id<npid;id++){
      for (int ipt=1;ipt<npt;ipt++){
        hv2EP[icent][0][id]           -> Add(hv2EP[icent][ipt][id]);
        hPT[icent][0][id]             -> Add(hPT[icent][ipt][id]);
        hv22pt[icent][0][id]          -> Add(hv22pt[icent][ipt][id]);
        hv24pt[icent][0][id]          -> Add(hv24pt[icent][ipt][id]);
        hcov22prime[icent][0][id]     -> Add(hcov22prime[icent][ipt][id]);
        hcov24prime[icent][0][id]     -> Add(hcov24prime[icent][ipt][id]);
        hcov42prime[icent][0][id]     -> Add(hcov42prime[icent][ipt][id]);
        hcov44prime[icent][0][id]     -> Add(hcov44prime[icent][ipt][id]);
        hcov2prime4prime[icent][0][id]-> Add(hcov2prime4prime[icent][ipt][id]);
        hv22ptGap[icent][0][id]       -> Add(hv22ptGap[icent][ipt][id]);
        hcov22primeGap[icent][0][id]  -> Add(hcov22primeGap[icent][ipt][id]);
        hcounter[icent][0][id]        -> Add(hcounter[icent][ipt][id]);        
      }
    }
  }
  ...
  */
  ...
  for (int icent=0; icent<ncent; icent++){ // loop over centrality classes
    ...
    v2e_RF[1][icent] = ev24;
    // EP
    v2_RF[2][icent] = hv2EP[icent][0][0]->GetBinContent(1)/sqrt(HRes[icent]->GetBinContent(1));
    v2e_RF[2][icent] = hv2EP[icent][0][0]->GetBinError(1)/sqrt(HRes[icent]->GetBinContent(1));
    // 2QC Gapped
    term cor2Gap = term(hv22Gap[icent]);
    double v22Gap = sqrt(cor2Gap.mVal);
    double ev22Gap = sqrt(1./(4.*cor2Gap.mVal)*cor2Gap.mMSE);
    ...
    // Differential flow calculation
    for (int id=0;id<npid;id++){   
      for(int ipt=0; ipt<1; ipt++){ // loop for all pT bin
        // vPt.push_back(hPT[icent][ipt][id] -> GetBinContent(1));
        // vPt.push_back((bin_pT[ipt]+bin_pT[ipt+1])/2.);
        // ePt.push_back(0);
        // v2EP
  ...
  std::vector<TGraphErrors*> vgrv2pt[npid];
    for (int id=0;id<npid;id++){
      if (bMergeCharged && id>3) continue;
      vgrv2pt[id].push_back(grDifFl[3][id]); // v2{gapped 2QC}
      for (int i=0; i<nmethod-1; i++){
        if (i==excludeMethod) continue;
  ...
  for (int id=0;id<npid;id++){
    if (bMergeCharged && id>3) continue;
    cV2PT[id] = (TCanvas*) DrawTGraph(vgrv2pt[id],"",rangeRatioRF.at(0).first, rangeRatioRF.at(0).second, mincent, maxcent, minV2int, maxV2int,
                                              coordinateLeg.at(0), coordinateLeg.at(1), coordinateLeg.at(2), coordinateLeg.at(3),
  ...
  vgrv2pt1040.push_back(grRefFl[3]);
  for (int imeth=0;imeth<nmethod-1;imeth++){
    if (imeth==excludeMethod) continue;
    vgrv2pt1040.push_back(grRefFl[imeth]);
  }
  cV2CentRF = (TCanvas*) DrawTGraph(vgrv2pt1040,"",rangeRatioRF.at(0).first, rangeRatioRF.at(0).second, mincent, maxcent, minV2int, maxV2int,
                                        coordinateLeg.at(0), coordinateLeg.at(1), coordinateLeg.at(2), coordinateLeg.at(3),
