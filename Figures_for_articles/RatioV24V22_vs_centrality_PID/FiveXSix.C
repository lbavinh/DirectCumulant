void FiveXSix(){
  ...
  const int nenergy = 6; // 4.5, 7.7, 11.5, 19.6, 27, 39 GeV
  const float mincent = -4;
  const float maxcent = 64.;
  const float maxV2Ratio = 1.3;
  const float minV2Ratio = 0.5;
  const float leg_coordinate[4]={0.4,0.2,0.7,0.5}; //  0.05,0.7,0.35,0.99
  ...
  TString legendEntries[]={"#pi^{#pm}","K^{#pm}","p"}; //(Phys.Rev.C.86.054908)
  ...
  int color[]={206,95,218,210,226,213};
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int imod=0;imod<nmodel;imod++){
      if (ien==0 && imod!=0 && imod!=1) continue; // at 4.5 GeV ony SMASH & UrQMD
      if (ien==3 && imod!=4) continue; // at 19.6 GeV only have vHLLE+UrQMD
      if (ien==4 && imod==1) continue;  // at 27 GeV only SMASH is missing
      if (ien==5 && imod!=2 && imod!=4) continue;  // at 39 GeV ony AMPT15 & vHLLE+UrQMD
      ...
      for (int imeth=0;imeth<nmethod;imeth++){ // v22, v24, v2(eta-sub), v22(eta-gap)
        ...
        grV2[ien][imod][imeth][1] = (TGraphErrors*)input[ien][imod]->Get(Form("grRF_%i_10",imeth));
        grV2[ien][imod][imeth][2] = (TGraphErrors*)input[ien][imod]->Get(Form("grRF_%i_3",imeth));
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int imod=0;imod<nmodel;imod++){
      for (int id=0;id<npid;id++){
        if      (ien==0 && imod!=0 && imod!=1) grRatioV2[ien][imod][id] = new TGraphErrors(2,xx,yy,xx,yy);
        else if (ien==3 && imod!=4)            grRatioV2[ien][imod][id] = new TGraphErrors(2,xx,yy,xx,yy); // at 19.6 GeV only have vHLLE+UrQMD
        else if (ien==4 && imod==1)            grRatioV2[ien][imod][id] = new TGraphErrors(2,xx,yy,xx,yy);  // at 27 GeV only SMASH is missing
        else if (ien==5 && imod!=2 && imod!=4) grRatioV2[ien][imod][id] = new TGraphErrors(2,xx,yy,xx,yy);  // at 39 GeV ony AMPT15 & vHLLE+UrQMD
        else{
  ...
  for (int imod=0;imod<nmodel;imod++){
    ...
    for (int ien=0;ien<nenergy;ien++){
      grRatioV2[ien][imod][0] -> SetMarkerStyle(kFullCircle);
      grRatioV2[ien][imod][1] -> SetMarkerStyle(kFullTriangleUp);
      grRatioV2[ien][imod][2] -> SetMarkerStyle(kOpenSquare);
      grRatioV2[ien][imod][0] -> SetMarkerColor(kRed+2);
      grRatioV2[ien][imod][0] -> SetLineColor(kRed+2);
      grRatioV2[ien][imod][1] -> SetMarkerColor(kBlue+3);
      grRatioV2[ien][imod][1] -> SetLineColor(kBlue+3);
  ...
  // gROOT->SetStyle("Pub");
  gStyle->SetErrorX(0);
  TCanvas *can = new TCanvas("can","can",200,10,1200,1500);
  can->SetLeftMargin(0.21);
  can->SetRightMargin(0.01);
  can->SetBottomMargin(0.21);
  can->SetTopMargin(0.19);

  TPaveLabel* title = new TPaveLabel(0.1,0.982,0.9,0.99,"Au+Au, 0.2<p_{T}<3.0 GeV/c");
  ...
  for (int imod=0;imod<nmodel;imod++){
    ...
    if (ien==5) can->cd(imod+26);
    if (ien==2 && imod==0) h[imod][ien] = new TH2F(Form("pad_%i_%i",imod,ien),";;v_{2}{4}/v_{2}{2}",1,mincent,maxcent,1,minV2Ratio,maxV2Ratio);
    else if (ien==5 && imod==2) h[imod][ien] = new TH2F(Form("pad_%i_%i",imod,ien),";centrality (%);",1,mincent,maxcent,1,minV2Ratio,maxV2Ratio);
    ...
    h[imod][ien]->GetXaxis()->SetLabelOffset(0.025);
    ...
    if (imod==0 && ien==5) tex.SetTextSize(labelSize1);
    // tex.DrawLatex(mincent+0.1,maxV2Ratio*0.98,padName[imod+ien].Data());
    if (ien==0) tex.DrawLatex(mincent+3,maxV2Ratio*0.97,Form("%s",padFancyName[imod].Data())); // #sqrt{s_{NN}}=
    if (imod==0) tex.DrawLatex(mincent+15,minV2Ratio+0.1,Form("%s GeV",energy[ien].Data()));
    ...
    if (ien==3 && imod==0) {
      ...
      leg_pt->AddEntry(grRatioV2[ien][imod][0],legendEntries[0].Data(),"p");
      leg_pt->AddEntry(grRatioV2[ien][imod][1],legendEntries[1].Data(),"p");
      leg_pt->AddEntry(grRatioV2[ien][imod][2],legendEntries[2].Data(),"p");
      leg_pt->Draw();      
    }
    TLine lineOne;
  ...
  can->SaveAs("Figure_Ratiov24v22_vs_centrality_PID_5x6.pdf");
  gROOT->SetStyle("Pub");
  can->SaveAs("Figure_Ratiov24v22_vs_centrality_PID_5x6.png");
