...
void AMPT::Booking(TString outFile){
  char name[800];
  char title[800];
  ...
  hMult = new TH1I("hMult", "Multiplicity distr;M;dN/dM", MAX_TRACKS, 0, MAX_TRACKS);
  hBimpvsMult = new TH2F("hBimpvsMult", "Impact parameter vs multiplicity;N_{ch};b (fm)", MAX_TRACKS, 0, MAX_TRACKS, 200, 0., 20.);
...
void AMPT::Loop_a_file(TString file){
  TStopwatch timer;
  timer.Start();
...
void AMPT::Loop_a_list_of_file(TString fileList){
  TStopwatch timer;
  timer.Start();
  ...
  if (chain == 0){
    cout << "TTree mctree is not found in " << line << endl;
    return;
  }
  Init(chain);
  Loop();
...
void AMPT::Loop()
{
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      Ana_event();
      if(jentry%10000==0) cout << "Event [" << jentry << "/" << nentries << "]" <<endl; // event counter
   }
}
...
// root -l -b -q anaFlow.C+'("/weekly/lbavinh/lbavinh/AMPT/split/AMPT15_7.7/runlist_AMPT15_7.7_55.list","test.root")'
