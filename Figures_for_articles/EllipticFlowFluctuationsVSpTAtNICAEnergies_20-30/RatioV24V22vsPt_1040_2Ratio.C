void RatioV24V22vsPt_1040_2Ratio(){
  ...
  TString legendEntries[2]={"Model", "Model, #eta-gap"};
    ...
    for (int imod=0;imod<nmodel;imod++){
      ...
      for (int imeth=0;imeth<nmethod;imeth++){ // v22, v24, v2(eta-sub), v22(eta-gap)
        for (int icent=0;icent<ncent;icent++){
          grV2[ien][imod][imeth][icent] = (TGraphErrors*)input[ien][imod]->Get(Form("gr_cent10-40_%i_8",imeth));
  ...
  for (int ien=0;ien<nenergy;ien++){
    for (int imod=0;imod<nmodel;imod++){
      for (int icent=0;icent<nmethod;icent++){
        ...
        if      (ien==2 && imod!=4)            grRatioV2[ien][imod][icent] = new TGraphErrors(2,xx,yy,xx,yy); // at 19.6 GeV only have vHLLE+UrQMD
          ...
          for (int i=0;i<nbins[ien][imod][icent][icent];i++){
            Double_t ratio = vy_gr[ien][imod][1][icent][i]/vy_gr[ien][imod][icent][icent][i];
            Double_t ratioErr = ratio*(TMath::Sqrt(TMath::Power(ey_gr[ien][imod][icent][icent][i]/vy_gr[ien][imod][icent][icent][i],2)+TMath::Power(ey_gr[ien][imod][1][icent][i]/vy_gr[ien][imod][1][icent][i],2)));
            vRatioV22GappedV24.push_back(ratio);
            vRatioV22GappedV24Err.push_back(ratioErr);
          }
          grRatioV2[ien][imod][icent] = new TGraphErrors(nbins[ien][imod][icent][icent],vx_gr[ien][imod][icent][icent],&vRatioV22GappedV24[0],ex_gr[ien][imod][icent][icent],&vRatioV22GappedV24Err[0]);      
        }
      }
    }
  }
  ...
  for (int imod=0;imod<nmodel;imod++){
    ...
    for (int ien=0;ien<nenergy;ien++){
      ...
      grRatioV2[ien][imod][0] -> SetMarkerStyle(kOpenCircle);
  ...
  TPaveLabel* title = new TPaveLabel(0.1,0.97,0.9,0.985,"Au+Au, 10-40%, Ch. hadrons");
  ...
  for (int imod=0;imod<nmodel;imod++){
    ...
    if (imod==1 && ien==0) {
      ...
      leg_pt->AddEntry(grRatioV2[ien][imod][3],legendEntries[1].Data(),"p");
      leg_pt->Draw();
    }
    TLine lineOne;
    ...
    // for (int icent=0;icent<ncent;icent++){
      // if (icent!=1) continue;
      grRatioV2[ien][imod][0]->Draw("P");
      grRatioV2[ien][imod][3]->Draw("P");
    // }
  
  }
  }
  can->SaveAs("Figure_Ratiov24v22_vs_pt_10-40_2Ratio.pdf");
  // gROOT->SetStyle("Pub");
  can->SaveAs("Figure_Ratiov24v22_vs_pt_10-40_2Ratio.png");
