...
void v2plot(TString model="UrQMD", TString energy="7.7"){
  TFile *fi = new TFile(Form("../ROOTFile/%s_%sGeV_v26v28.root",model.Data(),energy.Data()),"read");
  TFile *fo = new TFile(Form("v2_%s_%s_v26v28.root",model.Data(),energy.Data()),"recreate");
  ...
  c->Divide(1,2,0,0);
  c->cd(1);
  TH2F *h = new TH2F("h",Form("Au+Au at #sqrt{s_{NN}}=%s, %s, ch. hadrons, %1.1f<p_{T}<%1.1f GeV/c;centrality (%%);v_{2}", energy.Data(), model.Data(),pt_min_cut,pt_max_cut),NcentBins,centBinMin,centBinMax,1,v2Min,v2Max);
  ...
  c->SaveAs(Form("v2_%s_%s.pdf",model.Data(),energy.Data()));
  delete c;
  for(int c=0;c<maxCorrelator;c++){
    for(int icent=0;icent<NcentBins;icent++){
      delete recursion[c][icent];
    }
  }

  for (int i=0;i<6;i++){
    for (int c=0;c<NcentBins;c++){
      delete pCovariance[i][c];
    }
  }  
  fi->Close();
  fo->Close();
