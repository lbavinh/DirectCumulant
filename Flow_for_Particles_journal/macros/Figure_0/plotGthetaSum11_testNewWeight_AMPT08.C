...
void plotGthetaSum11_testNewWeight_AMPT08()
{
  TFile *fi = new TFile("../FirstRun_AMPT08_11.5_new.root","read");
  const int ncent = 9;
  const int rbins = 500;
  ...
  double rmax = 250;
  int thetaDraw = 2;
  int edge[ncent] = {1200,1200,790,550,490,460,480,45,40};
  TH1F *hGtheta[ncent];
  for (int ic=0; ic<ncent; ic++)
  {
  ...
  for (int ic=0; ic<ncent; ic++)
  {
    ...
    hGtheta[ic] -> GetXaxis()->SetRangeUser(0.,100);
  }
  ...
  tex.DrawLatex(90,0.2,"(b)");
  tex.DrawLatex(55,0.02,"#splitline{AMPT SM, #it{#sigma}_{p}=0.8 mb}{Au+Au at #sqrt{#it{s}_{NN}} = 11.5 GeV}");

  gPad->SetLogy();
  gStyle->SetOptStat(0);
  c.SaveAs("Gtheta115_testNewWeight_AMPT08.pdf");
  c.SaveAs("Gtheta115_testNewWeight_AMPT08.png");
}
